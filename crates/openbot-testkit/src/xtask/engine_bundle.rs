...
pub(crate) fn bundle(root: &Path) -> Result<()> {
    ...
    if release_epoch != 2 || protocol_version != 2 {
        bail!("engine bundle: engine pins release/protocol version drift from v2 contracts");
    }
    ...
    println!(
        "engine bundle: ok ({}; app.asar={} bytes; header_sha256={}; fuse_sentinels={fuse_count}; release_epoch=2)",
        destination.display(),
