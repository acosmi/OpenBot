...
> 真源 = `docs/2026-08-21-OpenBot全量Rust重写终版研究与实施方案.md`（**v4** = v3 就地修订至 §28.1 R125，2026-08-28；架构 / 能力 / 旅程）+ `docs/2026-08-22-OpenBot-GUI设计系统与视觉规格-方案.md`（**v2**；GUI 视觉 / token / 布局 / 主题 / i18n / a11y / 视觉闸门）。本文件只摘约束与理由，细节一律以两份方案章节为准；两者冲突时以方案为准（视觉归设计系统文档、架构归后端方案），并同 PR 修订本文件。`docs/2026-08-28-OpenBot-TauriGUI-ElectronChromium-GrokBot大面积Rust迁移-v4修订计划-用户裁决版.md` 已被 R115–R125 吸收，只作历史记录，不是实施依据；真源五层优先级见后端方案 §28.5。

---

## 1. 真源与现状

- **唯一实施真源**是上述方案 v4（§28 为修订记录；**R115–R125 是 2026-08-28 的架构裁决——范围冻结、`grok-bot` 定位与方法、Electron 获取与 shim、双 role engine、engine 进程 OS 约束、ExecutionRealm、runsc 判据、Linux Desktop tier-2、CSS 预算、台账 overlay、闸门与阶段——实施前必读**）。两份输入文档仓内不存在，只登记了 SHA-256；在 Phase 0 把原件归档到 `docs/inputs/` 之前，**不得**以"输入文档里写过"作为依据（§1.1）。
...
- **P0-code Batch52 已完成（2026-08-28）**：R124 exception-only overlay 已由 `parity-check` join 台账、按 branch diff 强制 done target revalidate，并机械打印 carry/revalidate/split/superseded = **1684/6/2/0**；`HumanLeaseEpoch::next` 改 checked，耗尽后清 lease、输入/acting fail-closed，只有 `ComputerGeneration` 前进才恢复；`grok-inventory --check` 同步钉死 tree 的 **2,110** 文件；`engine fetch|verify` 在 macOS arm64 对官方 zip 实得 **122,102,881 B / ee939d… / v43.3.0**；`electron-shim-check` 已锁文件/600 LOC/API/forbidden import/package/protocol hash 规则，P1 shim 缺席时明确报告；browser/components 新增 6 条 Engine T-ID。当前 browser-operations **7/43/50**、components **13/11/24**、parity **693/999/1692**；`openbot-computer` **9/0/0** 含 poisoned 用例，五个 P0 xtask 子命令均绿。**仍未实现且不得前移宣称**：P1 clean-room shim、`engine bundle`（rebrand/ASAR/fuses/integrity）、真 engine boot/role/render/confinement/conformance。`grok-bot/` tree 仍为 `86f5a85f560f721677fa7e587a67ac0ffc036cb5`，本批零改动；详见 `docs/2026-08-28-P0-code-batch52.md`。
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合**既有 schema/repository 地基；**W-3a/W-3b/W-4/W-6/W-7a/W-7b/W-7c**证据保持。Batch 11 后 G2 专项仍 **155/79/234**；Batch12–52新增面不改变该四元组。当前 tests **372/675/1047**、browser-operations **7/43/50**、env **49/25/74**、events **33/53/86**、tables **58/0**、API **66/103/169**、components **13/11/24**、routes **8/24/32**、UI **87/65/152**、整个 parity **693/999/1692**，fixtures **17/22/39**。R64–R68固定G3 data/realtime/migration；R69–R125固定Agent/G4/G5、GUI与Engine阶段门。仍未闭合三家trace、acting Approval完整集成、run-wide budget、Desktop Local OAuth、MCP admin/private-egress、完整AG-UI事件、用户创建Agent lifecycle、完整channel/home Composer/AppSidebar、sandbox args/JS/network/callback/MessageChannel正向执行、Electron/CDP/ScreenHub/viewer ticket/Desktop sandbox renderer、component admin正式journey/golden、GUI/golden/Tauri binary/window与经许可legacy drills。**其它未闭合项**：G4余项/G5余项/G7、G2外审/KMS/Windows、G6/G8（R52–R125）。
...
- §28.1 当前 **125** 条（R1–R125）。实施时除既有安全/G2裁决外，必须读 R61–R114 与 **R115–R125（2026-08-28 架构裁决）**：既有Linux/数据库/Agent/GUI裁决，以及reviewed Connected Accounts、compiled manifest/PG治理、Quote+Cards+Charts+Activity Gallery、component runtime授权、受控data read、ordinary component durable conversation、Activity follow-up exact retry、Decisions durable/runtime/renderer、sandboxed后端/Web fail-closed runtime与HumanLease/input protocol；AppSidebar/完整Composer、sandbox正向执行、Electron/CDP/ScreenHub/Desktop renderer、admin正式journey/golden与经许可legacy drills仍未完成。
...
Engine / 参考树闸门（R116 / R117 / R125）：P0-code 已落 `cargo xtask engine fetch|verify`（官方 zip sha256 + `--version`）、`cargo xtask electron-shim-check` 规则、`cargo xtask grok-inventory --check`、`parity/overlay/v4.yaml` 校验与工作区零 npm/package lock 反向扫描；P1 shim 尚未写，因此工作区（排除只读 `grok-bot/`）当前 `package.json` = 0，不能把“规则绿”冒充 shim 存在。`cargo xtask engine bundle` 的 fuses / ASAR integrity / rebrand / release epoch 与协议生成物 hash 实值随 P1 真 shim 落地，当前仍 todo。
