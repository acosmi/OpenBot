...
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  ② **acting before durable decision = 0 —— 通用 application 边界与真实 MCP/Memory/Drive 集合成员达成，整项仍未闭合**：`InvokeTool` 走 domain 十二段状态机；policy 结论只能由 domain 构造；decision+attempt commit 与 capability CAS 都发生在 `ToolControlPlane::execute(AuthorizedToolCall)` 之前；raw args 只能随字段私有信封到 executor；outcome+audit 同事务。Batch 11–13 把 official RMCP 3.1.4、callback、actor OAuth 与 Google Drive REST 真竖切接入同一管线，PG17 实得 success/definite failed、CEL/content/approval rejected、commit-unknown reconciliation、跨 replica sequence、refresh 与 401 单次 retry；acting MCP 在真实 approval 未接时明确拒绝。**仍不得宣称整项/G4 通过**：browser/file/shell、真实 approval UI 与第一次外部安全审计尚未闭合（R41/R74–R76）。
  ...
  **W-1/W-2 已闭合**既有 schema/repository 地基；**W-3a/W-3b/W-4/W-6/W-7a/W-7b/W-7c**证据保持。Batch 11 关闭 server-side-tools 五条后 G2 专项子队列为 **155/79/234**；Batch 12/13 的 OAuth/Drive 文件不属于这 21 个 G2 测试文件，所以四元组不变。当前 tests **265/782/1047**、env **49/25/74**、events **21/56/77**、tables **54/0**、API **35/122/157**、整个 parity **424/1237/1661**，fixtures **13/22/35**。R64–R68 固定 G3 native 数据/实时/迁移；R69–R76 固定 pure Agent、三 provider、journal、retry/token/audit、tool loop/remember、remote AG-UI/双凭据、RMCP、Server OAuth 与 Drive 生产面及未完成边界。仍未闭合三家 recorded/live trace、human approval、完整 run-wide budget、Desktop Local OAuth、MCP 完整 admin/private-egress、完整 AG-UI 事件投影、Memory GUI，以及实际 legacy export/production drills。**其它未闭合项**：G4 余项/G5/G7、G2 外审/KMS/Windows、G6/G8（R52–R76）。
...
- v3 §28.1 当前 **76** 条（R1–R76）。实施时除既有安全/G2 裁决外，必须读 R61–R76：Linux 历史证据、0016–0019、Actions manual-only、native thread/live/history/memory/run runtime/importer、Rust Agent、三 provider、retry/token/audit、tool loop/remember、remote AG-UI/双凭据、RMCP、Server OAuth 与 Google Drive REST 生产面及各自未完成边界。
