...
- **G2（Auth/Vault/Policy/Audit）本轮交付的是「不依赖网络、不依赖 C 工具链」的那整块**，四条判据（§24 G2）逐条如实：
  ...
  **W-1 已闭合**：native 0013 只 expand（`audit_events.prev_hash/row_hash` nullable + `audit_checkpoints/tool_calls/tool_attempts`），post fixture 31 表 / 248 列；并发施加恰好一次、对象碰撞全事务回滚、账本漂移 fail-closed。**W-2 current-schema 已闭合**：40 个规划 repo 中，已有物理表对应的 30 个全部实现并逐个跑 PG17；余下 10 个恰好对应 G3 尚未建表的 thread/run/outbox/memory/import，必须随建表同批，不造空壳。**其余未闭合项**：application 端口与用例；openbot-agent 工具调用接线；server 的生产 `AuthResolver`、`/metrics` 访问控制与 auth/admin 路由。
- v3 §28.1 当前 **38** 条（R1–R38）。实施时除 R23/R26–R34 外还必须读：**R35**（native migration 独立账本与并发）、**R36**（PG18 的 NOT NULL 约束表示不能冒充 PG17 oracle）、**R37**（闸门不得继承操作者日志过滤或 GNU/BSD 输出差异）、**R38**（repository 必须与物理表同批，禁止空壳占名）。
