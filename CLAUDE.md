...
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合** native/repository 地基；**W-3a** people 原子 slice、**W-3b** tool application boundary、**W-4 Server production slice** 已闭合；**W-6 `AUDIT_RETENTION_DAYS` 切换前预检**已闭合；**W-7a 环境 OIDC/safe-dialer** 与 **W-7b 动态 OIDC/SAML 代码竖切**已闭合。W-5 的 G2 队列由机器固定为 234 条；batch 4 后为 **100 done / 134 todo**：在 batch 3 的 65 条上新增 policy route 4/4 与 policy semantics 31/31；tests ledger 为 104/943，API 为 15/133。Policy GET/PUT 已穿 typed ApplicationService 与 R56 唯一 store；GET 未配置保留 null/default-deny，PUT 以 fresh admin+Origin 加固并由 PG HTTP 真腿证明 actor/重启。28 条 domain 求值 + 3 条唯一 Server parser 逐项闭合（R52–R57）。**仍未闭合项**：真实 G4 executor/Bot computer surface、G2 外审/KMS/跨平台、其它 application/API、G5 isolation。
...
- v3 §28.1 当前 **57** 条（R1–R57）。实施时除 R23/R26–R34 外还必须读 R35–R57，尤其 R42–R50 的既有安全边界、R51（迁移预检/TrimString）、R52–R56（G2 队列、身份/People/fresh/Policy/Audit）与 R57（Policy typed 管理写面、fresh/Origin 替代、31 条求值矩阵）。
