...
- **G1（Rust Core 与 PostgreSQL）四条判据本轮全部达成**（§24，四条缺一不可）：① 10 crate workspace + `cargo build --workspace --all-features --locked` 绿；② 同一个 `Arc<dyn ApplicationService>` 经 Axum 与 in-process 两条 transport 结果一致（`cargo test -p openbot-testkit --test transport_parity` = 7 passed，含"递到 port 上的调用逐字段相同"这条比结果更强的断言）；③ 28 表 / 13 migration 映射对走完 13 条 migration 的真参照库逐字段相等，read checksum 168/168 行逐字节相同（两条腿都在 PostgreSQL 侧渲染，避免"两边都用同一份 Rust 代码算"的自证）；④ tracing span + 关联字段 + 脱敏 + Prometheus metrics 从首个 vertical slice 生效。
  **W-4 已关掉四项旧缺口**：production PostgreSQL/单用户 `AuthResolver`、独立 `main.rs`、`/metrics` session 访问控制、MatchedPath route span/metrics 均已落地；people/audit/policy 命令另有同一 Arc 的 Axum/in-process 对拍；非 loopback 明文 HTTP 同时打启动告警并在 readiness 投影 `insecure_transport:true`。W-5 batch 7 已补 production Tenant Package loader/sync。G3 batch 1–5 依次落 native DB、thread/live、history/memory、run runtime/WS 与 importer。G4/G6 batch 6–15 已把 pure Rust Agent consumer、三 provider、package/managed 权威路由、fresh PG/Vault credential、journal、retry/token/audit、真实 multi-sampling tool loop + `remember`、package-backed remote AG-UI lifecycle/text、per-Agent callback 双凭据、official RMCP 3.1.4 catalog/grant/policy/audit/callback、Server/Desktop Remote credential-backed OAuth、Google Drive GA REST、durable human approval backend/API，以及 Leptos/Axum 可点击 Approval Web 竖切接入生产代码。**以下仍未闭合，不得算进去**：read checksum/Drizzle 账本深校验；三家 recorded/live vendor trace、Approval 真实 PG 浏览器端到端/critical realtime/thread 集成、完整 run-wide token/cost/并发/computer budget；Desktop Local installed-app client/system browser/random loopback callback、run/user cancellation 向 RMCP notification 传播、MCP 专用 private egress 与 admin custom/通用 refresh/effect/grant 完整 UI、用户创建 remote Agent lifecycle/customer auth、interrupt/resume 与其余事件 durable/UI projection；browser/file/shell；Memory GUI、实际 legacy exporter/production migration drills；active connections/OTel exporter；G5 前多用户 readiness 刻意 NotReady；package removed 生命周期；Tauri custom protocol、偏好持久化、全 route/component/golden/a11y。Batch 15 targeted：UI **19/0/0**、Server static **4/0/0**、UI WASM/Clippy、tools/i18n/design/css/bundle、Chromium APG/Approval/1024×640 与 strict recount **157/157/0** 全绿；浏览器 fixture 明示 test-only；cargo-vet 185 unvetted 保持红；没有运行 `cargo xtask ci`。fixture/native latest **0020**，见 R42/R43/R49–R78。
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合**既有 schema/repository 地基；**W-3a/W-3b/W-4/W-6/W-7a/W-7b/W-7c**证据保持。Batch 11 关闭 server-side-tools 五条后 G2 专项子队列为 **155/79/234**；Batch 12–15 的 OAuth/Drive/approval/UI 新增面不属于这 21 个 G2 测试文件，所以四元组不变。当前 tests **265/782/1047**、env **49/25/74**、events **26/56/82**、tables **55/0**、API **38/121/159**、UI **3/149/152**、整个 parity **436/1233/1669**，fixtures **14/22/36**。R64–R68 固定 G3 native 数据/实时/迁移；R69–R78 固定 pure Agent、三 provider、journal、retry/token/audit、tool loop/remember、remote AG-UI/双凭据、RMCP、Server OAuth、Drive、approval backend 与 Web GUI 地基及未完成边界。仍未闭合三家 recorded/live trace、Approval 真实 PG browser/critical realtime、完整 run-wide budget、Desktop Local OAuth/Tauri、MCP 完整 admin/private-egress、完整 AG-UI 事件投影、Memory GUI、完整 GUI/golden，以及实际 legacy export/production drills。**其它未闭合项**：G4 余项/G5/G7、G2 外审/KMS/Windows、G6/G8（R52–R78）。
  **D-1 已正式裁决**：RUSTSEC-2023-0071 只做一条窄豁免；`tools/check-rustsec-waivers.sh` 锁死精确四节生产依赖链、openidconnect feature 零扩张和 RSA 私钥符号零命中。Batch 15 另有两条仅 `informational=unmaintained`、`patched=[]` 的编译期宏窄豁免（paste/proc-macro-error2），由 `tools/check-ui-dependencies.sh` 锁 proc-macro 消费边界与修复状态；它们不冒充漏洞修复或维护性消失。cargo deny/audit 除三条显式裁决外为绿。见 R44/R78。
  **D-5 已建立棘轮**：CI 钉 `cargo-vet 0.10.0` 并跑 `cargo vet --locked`；Google exact/delta import 锁定 15 个 fully audited。350 个是 R45 bootstrap exemptions；W-7 TLS 新增 20 个、W-7b SAML 新增 30 个、G3 WebSocket 新增 3 个、G4 Batch 11 RMCP/schema 新增 32 个精确 exemption，逐条带 `owner=security` 与 `not a full source audit`，合计 **435**，不冒充审计。Batch 15 新 GUI 图出现 **185 unvetted** 后没有批量补 exemption，故当前 cargo-vet 明确红；新增/升级未覆盖版本仍当场判红。见 R45/R48/R50/R67/R74/R78。
...
- v3 §28.1 当前 **78** 条（R1–R78）。实施时除既有安全/G2 裁决外，必须读 R61–R78：Linux 历史证据、0016–0020、Actions manual-only、native thread/live/history/memory/run runtime/importer、Rust Agent、三 provider、retry/token/audit、tool loop/remember、remote AG-UI/双凭据、RMCP、Server OAuth、Google Drive REST、durable approval backend 与 Leptos/Axum GUI 地基及各自未完成边界。
...
- Batch 15 当前只完成 Web 地基：Axum static + strict CSP/首帧、Approval 可点击竖切、
  EmptyState/Badge/ThemeToggle 三条 ledger；LocaleSwitch 因偏好持久化未落仍 todo。浏览器 fixture
  只作 AX/键盘/视觉证据，不能替代 Batch 14 PostgreSQL approval 生产证据；完整 G6 仍未勾。
...
GUI 另加（设计系统文档 §15）：`cargo xtask tools verify` · `cargo test -p openbot-ui` · `xtask i18n-check` · `xtask design-lint` · `xtask css-check` · `xtask bundle-budget` · `bash tools/check-ui-dependencies.sh` · golden 截图（Web 110 张 / Desktop 每平台 54 张，差异像素 ≤ 0.1% 且无 8×8 全差异块；更新只能随 PR 附 diff 图人工批准）· CDP AX 树检查。Batch 15 的 cargo-vet 185 unvetted 未获明确授权不得批量 exemption。
