...
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合** native/repository 地基；**W-3a** people 原子 slice、**W-3b** tool application boundary、**W-4 Server production slice** 已闭合；**W-6 `AUDIT_RETENTION_DAYS` 切换前预检**已闭合；**W-7a 环境 OIDC/safe-dialer** 与 **W-7b 动态 OIDC/SAML 代码竖切**已闭合。W-5 的 G2 队列由机器固定为 234 条；batch 5 后为 **111 done / 123 todo**：在 batch 4 的 100 条上新增 tool-name 5/5 与 tool-result 6/6；tests ledger 为 115/932，API 仍为 15/133。纯 Rust/WASM-safe projection 不引 Leptos，并以 typed `policy_refused` 稳定码识别新拒绝；R51 TrimString 上收 contracts 后 domain 原路径重导出，实现仍唯一。亲读固定上游后，`server-side-tools` 5 条确认必须等 G4 的真 RMCP/catalog generation/stale grant/schema/policy/audit 竖切，继续 todo，不用 test-only runtime 冒充（R52–R58）。**仍未闭合项**：真实 G4 executor/Bot computer surface、G2 外审/KMS/跨平台、其它 application/API、G5 isolation。
...
- v3 §28.1 当前 **58** 条（R1–R58）。实施时除 R23/R26–R34 外还必须读 R35–R58，尤其 R42–R50 的既有安全边界、R51（迁移预检/TrimString）、R52–R57（G2 队列、身份/People/Policy/Audit）与 R58（tool projection 与真 G4 MCP 证据分界）。
