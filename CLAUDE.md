...
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合**既有 schema/repository 地基；**W-3a/W-3b/W-4/W-6/W-7a/W-7b/W-7c**证据保持。G2 队列仍 **149/85**。G3 batch 3 后 tests **169/878**、tables **54/0**、API **25/130/155**、整个 parity **287/1365/1652**，fixtures **10/22**。R64 固定 native scope status；R65 固定 transaction/replay/SSE；R66 固定 history empty-vs-error、memory origin/owner 构造边界、correct/supersede、content erase、FTS+structured-tag scope recall 与无后台抽取。仍未闭合 WebSocket、provider accumulator 调用、outbox/lease recovery/terminal writer、remember tool、Memory GUI、importer。**其它未闭合项**：真实 G4/G5/G7、G2 外审/KMS/Windows、G6/G8（R52–R66）。
...
- v3 §28.1 当前 **66** 条（R1–R66）。实施时除既有安全/G2 裁决外，必须读 R61–R66：Linux 历史证据、0016、Actions manual-only、mint/status、transaction/live、history/explicit memory 及各自未完成边界。
