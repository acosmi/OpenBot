...
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合**既有 schema/repository 地基；**W-3a/W-3b/W-4/W-6/W-7a/W-7b/W-7c**证据保持。Batch 11 后 G2 专项仍 **155/79/234**；Batch12–40新增面不改变该四元组。当前 tests **372/675/1047**、env **49/25/74**、events **26/56/82**、tables **56/0**、API **54/113/167**、components **1/21/22**、routes **8/24/32**、UI **86/66/152**、整个 parity **652/1026/1678**，fixtures **16/22/38**。R64–R68固定G3 data/realtime/migration；R69–R103固定Agent/G4与GUI批次、session/channel realtime/detail/shared thread/ChannelRow/Agent roster/create/routing/Composer/conversation/durable cancel/Memory Controls/Settings/Connected Accounts/Components Gallery及未完成边界。仍未闭合三家trace、Approval完整集成、run-wide budget、Desktop Local OAuth、MCP admin/private-egress、完整AG-UI事件、用户创建Agent lifecycle、完整channel/home Composer/AppSidebar、其余12 compiled renderer/runtime grant、GUI/golden/Tauri binary/window与经许可legacy drills。**其它未闭合项**：G4余项/G5/G7、G2外审/KMS/Windows、G6/G8（R52–R103）。
...
- v3 §28.1 当前 **103** 条（R1–R103）。实施时除既有安全/G2裁决外，必须读R61–R103：Linux历史证据、0016–0022、Actions manual-only、native thread/live/history/memory/run runtime/importer、Rust Agent/G4、GUI地基/Tauri、27primitive/46图标、layout/AgentPresence/线稿、session sign-out、channel activity/detail/shared thread/ChannelRow、Agent profile/roster、channel create/routing/native first turn、Composer draft/queue、channel snapshot/realtime/idle send、durable跨副本cancel/production queue、actor memory write control/GUI、Settings偏好/stable owner/secondary shell、reviewed Connected Accounts，以及compiled manifest/PG治理/Quote Gallery read slice；AppSidebar/完整Composer/其余renderer/golden/经许可legacy drills仍未完成。
