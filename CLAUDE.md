...
- **G2（Auth/Vault/Policy/Audit）本轮交付的是「不依赖网络、不依赖 C 工具链」的那整块**，四条判据（§24 G2）逐条如实：
  ...
  **W-1 已闭合**：native 0013 只 expand（`audit_events.prev_hash/row_hash` nullable + `audit_checkpoints/tool_calls/tool_attempts`），post fixture 31 表 / 248 列；并发施加恰好一次、对象碰撞全事务回滚、账本漂移 fail-closed。**W-2 current-schema 已闭合**：40 个规划 repo 中，已有物理表对应的 30 个全部实现并逐个跑 PG17；余下 10 个恰好对应 G3 尚未建表的 thread/run/outbox/memory/import，必须随建表同批，不造空壳。**W-3a people slice 已闭合**：native 0014、五个 typed command/use case 与 PostgreSQL 原子 adapter 落地。**W-3b tool application 边界已闭合**：application 7 条 + Agent gateway 3 条 + PG17 journal 5 条主矩阵（另有 contracts/domain guard）落地，pipeline 四条 events 台账转 done。**仍未闭合项**：session/group/IdP/credentials/tenant package 等 application 面；server 生产 `AuthResolver`、fresh-session/CSRF/origin、`/metrics` 访问控制与 auth/admin 路由；G4 production executor。
- v3 §28.1 当前 **41** 条（R1–R41）。实施时除 R23/R26–R34 外还必须读：**R35**（native migration 独立账本与并发）、**R36**（PG18 的 NOT NULL 约束表示不能冒充 PG17 oracle）、**R37**（闸门不得继承操作者日志过滤或 GNU/BSD 输出差异）、**R38**（repository 必须与物理表同批，禁止空壳占名）、**R39**（auth generation 的 0014 持久化边界）、**R40**（people typed port 与原子事务边界）、**R41**（tool application/capability/journal/unknown 边界）。
