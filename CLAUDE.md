...
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合**既有 schema/repository 地基；**W-3a/W-3b/W-4/W-6/W-7a/W-7b/W-7c**证据保持。Batch 11 关闭 server-side-tools 五条后 G2 专项子队列为 **155/79/234**；Batch 12–26 的 OAuth/Drive/approval/UI 新增面不属于这 21 个 G2 测试文件，所以四元组不变。当前 tests **265/782/1047**、env **49/25/74**、events **26/56/82**、tables **56/0**、API **40/121/161**、UI **79/73/152**、整个 parity **515/1157/1672**，fixtures **15/22/37**。R64–R68 固定 G3 native 数据/实时/迁移；R69–R89 固定 pure Agent、三 provider、journal、retry/token/audit、tool loop/remember、remote AG-UI/双凭据、RMCP、Server OAuth、Drive、approval backend、Web GUI、偏好/Tauri、27primitive、46icon mapping、4layout业务、2AgentPresence映射及未完成边界。仍未闭合三家 recorded/live trace、Approval 真实 PG browser/critical realtime、完整 run-wide budget、Desktop Local OAuth、MCP 完整 admin/private-egress、完整 AG-UI 事件投影、Memory GUI、完整 GUI/golden、Tauri binary/window lifecycle，以及实际 legacy export/production drills。**其它未闭合项**：G4 余项/G5/G7、G2 外审/KMS/Windows、G6/G8（R52–R89）。
...
- v3 §28.1 当前 **89** 条（R1–R89）。实施时除既有安全/G2 裁决外，必须读 R61–R89：Linux历史证据、0016–0021、Actions manual-only、native thread/live/history/memory/run runtime/importer、Rust Agent、三provider、retry/token/audit、tool loop/remember、remote AG-UI/双凭据、RMCP、Server OAuth、Google Drive REST、durable approval、Leptos/Axum GUI、偏好/Tauri target graph、27primitive全done、46icon mapping done/brand todo、4layout业务+2AgentPresence映射done及其余业务/golden未完成边界。
...
- Batch 15–26 已完成 Web 地基、Approval 可点击竖切、Server/Desktop 偏好、opt-in Tauri、
  27条primitive、46条Lucide mapping、4条layout业务与2条AgentPresence映射；UI ledger当前79/73。浏览器
