...
- **G1（Rust Core 与 PostgreSQL）四条判据本轮全部达成**（§24，四条缺一不可）：① 10 crate workspace + `cargo build --workspace --all-features --locked` 绿；② 同一个 `Arc<dyn ApplicationService>` 经 Axum 与 in-process 两条 transport 结果一致（`cargo test -p openbot-testkit --test transport_parity` = 7 passed，含"递到 port 上的调用逐字段相同"这条比结果更强的断言）；③ 28 表 / 13 migration 映射对走完 13 条 migration 的真参照库逐字段相等，read checksum 168/168 行逐字节相同（两条腿都在 PostgreSQL 侧渲染，避免"两边都用同一份 Rust 代码算"的自证）；④ tracing span + 关联字段 + 脱敏 + Prometheus metrics 从首个 vertical slice 生效。
  **W-4 已关掉四项旧缺口**：production PostgreSQL/单用户 `AuthResolver`、独立 `main.rs`、`/metrics` session 访问控制、MatchedPath route span/metrics 均已落地；people/audit/policy 命令另有同一 Arc 的 Axum/in-process 对拍；非 loopback 明文 HTTP 同时打启动告警并在 readiness 投影 `insecure_transport:true`。W-5 batch 7 已补 production Tenant Package loader/sync。G3 batch 1–5 依次落 native DB、thread/live、history/memory、run runtime/WS 与 importer。G4 batch 6–14 已把 pure Rust Agent consumer、三 provider、package/managed 权威路由、fresh PG/Vault credential、journal、retry/token/audit、真实 multi-sampling tool loop + `remember`、package-backed remote AG-UI lifecycle/text、per-Agent callback 双凭据、official RMCP 3.1.4 catalog/grant/policy/audit/callback、Server/Desktop Remote credential-backed OAuth、Google Drive GA REST，以及 durable human approval backend/API 接入 Server main。**以下仍未闭合，不得算进去**：read checksum/Drizzle 账本深校验；三家 recorded/live vendor trace、可点击 Leptos approval GUI、完整 run-wide token/cost/并发/computer budget；Desktop Local installed-app client/system browser/random loopback callback、run/user cancellation 向 RMCP notification 传播、MCP 专用 private egress 与 admin custom/通用 refresh/effect/grant 完整 UI、用户创建 remote Agent lifecycle/customer auth、interrupt/resume 与其余事件 durable/UI projection；browser/file/shell；Memory GUI、实际 legacy exporter/production migration drills；active connections/OTel exporter；G5 前多用户 readiness 刻意 NotReady；package removed 生命周期。Batch 14 targeted：native0020 PG17/SCRAM **2/0/0**、approval runtime **1/0/0**、MCP **5/0/0**（含 acting grant+deny）、Server framing **1/0/0**、domain/application/UI pure 全绿；七 crate Clippy与 contracts/UI WASM、strict recount 全绿；没有运行 `cargo xtask ci`。fixture/native latest **0020**，见 R42/R43/R49–R77。
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  ② **acting before durable decision = 0 —— 通用 application 边界与真实 MCP/Memory/Drive 集合成员达成，整项仍未闭合**：`InvokeTool` 走 domain 十二段状态机；policy 结论只能由 domain 构造；decision+attempt commit 与 capability CAS 都发生在 `ToolControlPlane::execute(AuthorizedToolCall)` 之前；raw args 只能随字段私有信封到 executor；outcome+audit 同事务。Batch 11–14 把 official RMCP、actor OAuth、Drive 与 durable approval 真竖切接入同一管线；acting MCP 实得 pending→human grant→approval-linked decision/attempt→vendor success，以及 human deny→attempt/vendor 0。**仍不得宣称整项/G4 通过**：browser/file/shell、可点击 Leptos approval GUI 与第一次外部安全审计尚未闭合（R41/R74–R77）。
  ...
  **W-1/W-2 已闭合**既有 schema/repository 地基；**W-3a/W-3b/W-4/W-6/W-7a/W-7b/W-7c**证据保持。Batch 11 关闭 server-side-tools 五条后 G2 专项子队列为 **155/79/234**；Batch 12–14 的 OAuth/Drive/approval 新增面不属于这 21 个 G2 测试文件，所以四元组不变。当前 tests **265/782/1047**、env **49/25/74**、events **26/56/82**、tables **55/0**、API **37/122/159**、整个 parity **432/1237/1669**，fixtures **14/22/36**。R64–R68 固定 G3 native 数据/实时/迁移；R69–R77 固定 pure Agent、三 provider、journal、retry/token/audit、tool loop/remember、remote AG-UI/双凭据、RMCP、Server OAuth、Drive 与 approval backend 生产面及未完成边界。仍未闭合三家 recorded/live trace、approval GUI、完整 run-wide budget、Desktop Local OAuth、MCP 完整 admin/private-egress、完整 AG-UI 事件投影、Memory GUI，以及实际 legacy export/production drills。**其它未闭合项**：G4 余项/G5/G7、G2 外审/KMS/Windows、G6/G8（R52–R77）。
...
- v3 §28.1 当前 **77** 条（R1–R77）。实施时除既有安全/G2 裁决外，必须读 R61–R77：Linux 历史证据、0016–0020、Actions manual-only、native thread/live/history/memory/run runtime/importer、Rust Agent、三 provider、retry/token/audit、tool loop/remember、remote AG-UI/双凭据、RMCP、Server OAuth、Google Drive REST 与 durable approval backend 生产面及各自未完成边界。
