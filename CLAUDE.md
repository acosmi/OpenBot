...
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合** native/repository 地基；**W-3a** people 原子 slice、**W-3b** tool application boundary、**W-4 Server production slice** 已闭合；**W-6 `AUDIT_RETENTION_DAYS` 切换前预检**已闭合；**W-7a 环境 OIDC/safe-dialer** 与 **W-7b 动态 OIDC/SAML 代码竖切**已闭合。W-5 的 G2 队列由机器固定为 234 条；identity batch 1 后为 **36 done / 198 todo**（dev-actor 3/3、Entra 5/5、single-user 6/6、roles 15/16，另含既有 encrypt-sso 7/7）。单用户 canonical id 已恢复为兼容键 `dev-local-user`、角色唯一 admin；fresh baseline + native ledger 已原子化且真实二启 exit 0（R52–R54）。**仍未闭合项**：G2 外审/KMS/跨平台、其它 application/API、G4 executor、G5 isolation。
...
- v3 §28.1 当前 **54** 条（R1–R54）。实施时除 R23/R26–R34 外还必须读 R35–R54，尤其 R42–R47 的既有边界、R48–R50（safe dialer/OIDC/SAML）、R51（审计留存迁移预检/ECMAScript TrimString）、R52–R53（G2 队列/单用户 canonical identity）与 R54（fresh 数据库可重启且 bootstrap 原子）。
