...
- **G1（Rust Core 与 PostgreSQL）四条判据本轮全部达成**（§24，四条缺一不可）：① 10 crate workspace + `cargo build --workspace --all-features --locked` 绿；② 同一个 `Arc<dyn ApplicationService>` 经 Axum 与 in-process 两条 transport 结果一致（`cargo test -p openbot-testkit --test transport_parity` = 7 passed，含"递到 port 上的调用逐字段相同"这条比结果更强的断言）；③ 28 表 / 13 migration 映射对走完 13 条 migration 的真参照库逐字段相等，read checksum 168/168 行逐字节相同（两条腿都在 PostgreSQL 侧渲染，避免"两边都用同一份 Rust 代码算"的自证）；④ tracing span + 关联字段 + 脱敏 + Prometheus metrics 从首个 vertical slice 生效。
  **以下仍未闭合，不得算进 G1**：`AuthResolver` 没有生产实现（属 G2 的 method/origin 面），所以还没有可独立运行的 Server 二进制；`/metrics` 的访问控制同属 G2；read checksum 只覆盖 seed 造出来的取值形态，不是全量数据证明；`InfraError::Connect` 只实测过鉴权失败一种形态；上游 Drizzle 账本仍只数条目不校验内容（Rust-owned 账本已按 SQL SHA-256 校验，R35）；跨 transport 对拍跑在 fake port 上（真库那条腿的 harness 在 infra 侧）；`subscribe` / `AppEventStream` 不在对拍矩阵内；span / metrics 尚无 `route` 维度。**PostgreSQL 17 差异已闭合**：17.11 真库重生成 0012/0013/0014 fixture，完整 infra 真库矩阵 248 passed，见 R36/R38–R40。
- **G2（Auth/Vault/Policy/Audit）本轮交付的是「不依赖网络、不依赖 C 工具链」的那整块**，四条判据（§24 G2）逐条如实：
  ...
  ② **acting before durable decision = 0 —— 类型状态机 + 存储 + repository 达成，端到端尚未闭合**：`openbot_domain::tool::pipeline` 的 5 条 `compile_fail` 钉住构造性边界；native 0013 落 `tool_calls` / `tool_attempts`；`ToolCallRepo::record_first_decision` 在同一事务写 call+attempt，commit 后才签发 `DurableDecisionReceipt`，重复 attempt 使 call 零残留；attempt 状态以 capability CAS 迁移，unknown 固定进 reconciliation。**仍缺 tool application port/use case 与 openbot-agent 调用链接线**；people application slice 的闭合不能替代这条工具端到端判据。
  ...
  **W-1 已闭合**：native 0013 只 expand（`audit_events.prev_hash/row_hash` nullable + `audit_checkpoints/tool_calls/tool_attempts`），post fixture 31 表 / 248 列；并发施加恰好一次、对象碰撞全事务回滚、账本漂移 fail-closed。**W-2 current-schema 已闭合**：40 个规划 repo 中，已有物理表对应的 30 个全部实现并逐个跑 PG17；余下 10 个恰好对应 G3 尚未建表的 thread/run/outbox/memory/import，必须随建表同批，不造空壳。**W-3 people application slice 已闭合**：native 0014 持久化 user auth generation；五个 typed command/use case 与 PostgreSQL 原子 adapter 落地；并发 last-admin、audit rollback、revoke/restore、cursor/search 在 PG17 5/5。**仍未闭合项**：其他 application 能力，尤其 tool port/use case 与 openbot-agent 调用链；server 的生产 `AuthResolver`、fresh-session/CSRF/origin、`/metrics` 访问控制与 auth/admin 路由。
- v3 §28.1 当前 **40** 条（R1–R40）。实施时除 R23/R26–R34 外还必须读：**R35**（native migration 独立账本与并发）、**R36**（PG18 的 NOT NULL 约束表示不能冒充 PG17 oracle）、**R37**（闸门不得继承操作者日志过滤或 GNU/BSD 输出差异）、**R38**（repository 必须与物理表同批，禁止空壳占名）、**R39**（auth generation 的 0014 持久化边界）、**R40**（people typed port 与原子事务边界）。
