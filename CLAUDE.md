...
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合** native/repository 地基；**W-3a** people 原子 slice、**W-3b** tool application boundary、**W-4 Server production slice** 已闭合；**W-7a 环境 OIDC/safe-dialer** 与 **W-7b 动态 OIDC/SAML 代码竖切**已闭合。**仍未闭合项**：G2 外审/KMS/跨平台、其它 application/API、G4 executor、G5 isolation。
...
- v3 §28.1 当前 **50** 条（R1–R50）。实施时除 R23/R26–R34 外还必须读 R35–R50，尤其 R42–R47 的既有边界、R48–R49（safe dialer/OIDC）和 R50（SAML/xmlsec FFI、动态 IdP 与尚未闭合外审/KMS/跨平台）。
