...
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  ④ **OIDC/SAML/session/role/group/revoke 全矩阵 —— 生产代码与本机矩阵已接通，整项仍因外审/跨平台未闭合**：W-7a 有唯一 safe dialer、环境三家 OIDC、跨副本 state/token/JWKS/group/keyed session；W-7b 已接动态 admin 注册/更新/删除、匿名 HMAC routing ticket、动态 OIDC 跨 replica、SAML SP metadata/Redirect+POST/ACS、根级 XMLDSig、SHA-2 allowlist、Destination/Audience/Recipient/request/time/replay、account-anchor cleanup 与 v2 SSO config。**仍缺第一次独立 SAML/XSW 与整体安全外审、Linux CI 真跑、Windows 原生构建、Server KMS/HSM；需要 SP private key 的 signed AuthnRequest 也必须与 KMS 同批，不在当前输入面假装支持**，不得宣称 G2 全矩阵/整关闭合（R48–R50）。
  **W-1/W-2 已闭合** native/repository 地基；**W-3a** people 原子 slice、**W-3b** tool application boundary、**W-4 Server production slice** 已闭合；**W-6 `AUDIT_RETENTION_DAYS` 切换前预检**已闭合；**W-7a 环境 OIDC/safe-dialer** 与 **W-7b 动态 OIDC/SAML 代码竖切**已闭合。W-5 的 G2 队列由机器固定为 234 条；batch 7 后为 **149 done / 85 todo**：在 batch 6 的 per-user credential 之后，`tenant-package` 26/26 与 fintech fixture 1/1 全部闭合；tests ledger 为 153/894，API 仍为 15/133，整个 parity 为 251/1394。runtime tenant CSS 按 GUI 第一真源移除，五 YAML 经显式 env allowlist、有界 loader 与 application-owned PostgreSQL sync；`all`/single-user/named group 进入 materialized membership，audience 收紧同步推进 generation/清 session；neutral fixture 保留 MIT provenance。剩余 85 条均依赖 G4/G5/G7 真生产面，不能用 fake runtime 冒充（R52–R60）。**仍未闭合项**：真实 G4 executor/Bot computer surface、G2 外审/KMS/跨平台、package removed-entity 生命周期、其它 application/API、G5 isolation。
...
- v3 §28.1 当前 **60** 条（R1–R60）。实施时除 R23/R26–R34 外还必须读 R35–R60，尤其 R42–R50 的既有安全边界、R51（迁移预检/TrimString）、R52–R57（G2 队列、身份/People/Policy/Audit）、R58（tool projection 与真 G4 MCP 证据分界）、R59（per-user credential 选择/退役）与 R60（Tenant Package/theme/group/brand 裁决）。
