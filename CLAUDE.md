...
- **2026-08-28 v4 就地修订（R115–R125）已落档；代码面只动了两处**：`xtask bundle-budget` 的 CSS 上限 128 KiB / 120 KiB 警戒（R123）与 `parity/components.yaml` T-CMP-0021、`parity/browser-operations.yaml` T-BROP-0046 的 target/owner（R124）。以下是 **P0-code 批次**（后端方案 §19.1）的待做项，尚未实现，不得当作已存在：`parity/overlay/v4.yaml` + xtask 校验、`HumanLeaseEpoch` 的 checked / poisoned 修复、`cargo xtask grok-inventory`、`cargo xtask engine fetch|verify|bundle`（消费 `tools/engine-pins.toml`）、`cargo xtask electron-shim-check`、engine 相关新 T-ID。`grok-bot/research-archives` 的两个 LFS 指针已移除（对象从未上传，默认 clone 恒红），参考树 hash 见后端方案 §1.2。
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合**既有 schema/repository 地基；**W-3a/W-3b/W-4/W-6/W-7a/W-7b/W-7c**证据保持。Batch 11 后 G2 专项仍 **155/79/234**；Batch12–51新增面不改变该四元组。当前 tests **372/675/1047**、browser-operations **7/39/46**、env **49/25/74**、events **33/53/86**、tables **58/0**、API **66/103/169**、components **13/9/22**、routes **8/24/32**、UI **87/65/152**、整个 parity **693/993/1686**，fixtures **17/22/39**。R64–R68固定G3 data/realtime/migration；R69–R114固定Agent/G4/G5与GUI批次及未完成边界。仍未闭合三家trace、acting Approval完整集成、run-wide budget、Desktop Local OAuth、MCP admin/private-egress、完整AG-UI事件、用户创建Agent lifecycle、完整channel/home Composer/AppSidebar、sandbox args/JS/network/callback/MessageChannel正向执行、Electron/CDP/ScreenHub/viewer ticket/Desktop sandbox renderer、component admin正式journey/golden、GUI/golden/Tauri binary/window与经许可legacy drills。**其它未闭合项**：G4余项/G5余项/G7、G2外审/KMS/Windows、G6/G8（R52–R114）。
...
Engine / 参考树闸门（R116 / R117 / R125；**P0-code 落地前全部 todo**）：`cargo xtask engine fetch|verify|bundle`（消费 `tools/engine-pins.toml`：sha256、`--version`、fuses、ASAR integrity、rebrand、release epoch）· `cargo xtask electron-shim-check`（文件 allowlist、非空 LOC ≤ 600、Electron/Node API allowlist、forbidden import / string、协议 hash）· `cargo xtask grok-inventory --check`（`inventory/grok/files.yaml` 与参考树同步）· 仓内 `package.json` 恰一个且零 dependencies / scripts 的反向 grep · `parity/overlay/v4.yaml` 校验。
