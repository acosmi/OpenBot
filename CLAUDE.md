...
- **G1（Rust Core 与 PostgreSQL）四条判据本轮全部达成**（§24，四条缺一不可）：① 10 crate workspace + `cargo build --workspace --all-features --locked` 绿；② 同一个 `Arc<dyn ApplicationService>` 经 Axum 与 in-process 两条 transport 结果一致（`cargo test -p openbot-testkit --test transport_parity` = 7 passed，含"递到 port 上的调用逐字段相同"这条比结果更强的断言）；③ 28 表 / 13 migration 映射对走完 13 条 migration 的真参照库逐字段相等，read checksum 168/168 行逐字节相同（两条腿都在 PostgreSQL 侧渲染，避免"两边都用同一份 Rust 代码算"的自证）；④ tracing span + 关联字段 + 脱敏 + Prometheus metrics 从首个 vertical slice 生效。
  **W-4 已关掉四项旧缺口**：production PostgreSQL/单用户 `AuthResolver`、独立 `main.rs`、`/metrics` session 访问控制、MatchedPath route span/metrics 均已落地；people/audit/policy 命令另有同一 Arc 的 Axum/in-process 对拍；非 loopback 明文 HTTP 同时打启动告警并在 readiness 投影 `insecure_transport:true`。W-5 batch 7 已补 production Tenant Package loader/sync。G3 batch 1–5 依次落 native DB、thread/live、history/memory、run runtime/WS 与 importer。G4 batch 6–10 已把 pure Rust Agent consumer、三 provider、package/managed 权威路由、fresh PG/Vault credential、journal、retry/token/audit、第一条真实 multi-sampling tool loop + `remember` executor、package-backed remote AG-UI lifecycle/text，以及 per-Agent callback token + signed empty-tool-set assertion/refusal 链接入 Server main。**以下仍未闭合，不得算进去**：read checksum/Drizzle 账本深校验；三家 recorded/live vendor trace、human proof-of-intent/approval GUI、完整 run-wide token/cost/并发/computer budget；非空 remote tool grant/callback success、跨副本 call sequence、outbound auth header、用户创建 remote Agent lifecycle、interrupt/resume 与其余事件 durable/UI projection；RMCP/Drive/browser/file/shell；Memory GUI、实际 legacy exporter/production migration drills；active connections/OTel exporter；G5 前多用户 readiness 刻意 NotReady；package removed 生命周期。R64 前最后一次完整 workspace **1101/0/117**、PG17/SCRAM **525/0/0**；R73 当前只跑本机定向，完整新总数不猜。fixture/native latest **0016**，见 R42/R43/R49–R73。
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合**既有 schema/repository 地基；**W-3a/W-3b/W-4/W-6/W-7a/W-7b/W-7c**证据保持。G2 队列因 callback lifecycle audit 所在文件新增一条，现 **150/84**。G4 batch 10 后 tests **206/841**、env **49/25/74**、events **15/62/77**、tables **54/0**、API **28/128/156**、整个 parity **352/1308/1660**，fixtures **10/22**。R64–R68 固定 G3 native 数据/实时/迁移；R69–R73 固定 pure Agent、三 provider、journal、retry/token/audit、tool loop/remember、remote AG-UI 文本与双凭据空工具集拒绝链及未完成边界。仍未闭合三家 recorded/live trace、human approval、完整 run-wide budget、非空 remote grant/callback outcome/跨副本 sequence/完整事件投影、RMCP/Drive、Memory GUI，以及实际 legacy export/production drills。**其它未闭合项**：G4 余项/G5/G7、G2 外审/KMS/Windows、G6/G8（R52–R73）。
  ...
  **D-5 已建立棘轮**：CI 钉 `cargo-vet 0.10.0` 并跑 `cargo vet --locked`；Google exact/delta import 锁定 15 个 fully audited。350 个是 R45 bootstrap exemptions；W-7 TLS 新增 20 个、W-7b SAML 新增 30 个、G3 WebSocket 新增 3 个精确 exemption，逐条带 `owner=security` 与 `not a full source audit`，合计 **403**，不冒充审计。新增/升级未覆盖版本仍当场判红。见 R45/R48/R50/R67。
...
- v3 §28.1 当前 **73** 条（R1–R73）。实施时除既有安全/G2 裁决外，必须读 R61–R73：Linux 历史证据、0016、Actions manual-only、native thread/live/history/memory/run runtime/importer、Rust Agent、三 provider、retry/token/audit、tool loop/remember、remote AG-UI 文本/双凭据拒绝链及各自未完成边界。
