...
- **G2（Auth/Vault/Policy/Audit）已从纯规则推进到真实 OIDC/SAML 网络/FFI 竖切**，四条判据（§24 G2）逐条如实：
  ...
  **W-1/W-2 已闭合**既有 schema/repository 地基；**W-3a/W-3b/W-4/W-6/W-7a/W-7b/W-7c**证据保持。G2 队列仍 **149/85**。G3 batch 4 后 tests **169/878**、tables **54/0**、API **26/130/156**、整个 parity **288/1365/1653**，fixtures **10/22**。R64 固定 native scope status；R65 固定 transaction/replay/SSE；R66 固定 history/explicit memory；R67 固定 expected-sequence writer、outbox reserve→ack→activate、fencing recovery、fail-closed consumer 与 same-origin WebSocket。仍未闭合真实 provider→`DurableTextRun` producer、真实 Agent reducer/tool loop、remember tool、Memory GUI、importer。**其它未闭合项**：真实 G4/G5/G7、G2 外审/KMS/Windows、G6/G8（R52–R67）。
...
- v3 §28.1 当前 **67** 条（R1–R67）。实施时除既有安全/G2 裁决外，必须读 R61–R67：Linux 历史证据、0016、Actions manual-only、mint/status、transaction/live、history/explicit memory、run runtime/WebSocket 及各自未完成边界。
