...
- **G2（Auth/Vault/Policy/Audit）本轮交付的是「不依赖网络、不依赖 C 工具链」的那整块**，四条判据（§24 G2）逐条如实：
  ...
  ② **acting before durable decision = 0 —— 类型状态机 + 存储地基达成，端到端尚未闭合**：`openbot_domain::tool::pipeline` 的 5 条 `compile_fail` 钉住「没有 durable decision 拿不到 capability」等构造性边界；native 0013 已以自有 SHA-256 账本落 `tool_calls` / `tool_attempts`，并由 PostgreSQL 17 并发、FK、原子回滚测试验收。**仍缺 W-2 repository 把 decision+attempt 同事务写入并签发 `DurableDecisionReceipt`，以及 application/agent 调用链接线**；在那之前不得宣称 G2 此判据闭合。
  ...
  **W-1 已闭合**：native 0013 只 expand（`audit_events.prev_hash/row_hash` nullable + `audit_checkpoints/tool_calls/tool_attempts`），post fixture 31 表 / 248 列；并发施加恰好一次、对象碰撞全事务回滚、账本漂移 fail-closed。**其余未闭合项**：infra repos（people / session / role / revoked / sso / credentials / action_policy / audit / tools）；application 端口与用例；server 的生产 `AuthResolver`、`/metrics` 访问控制与 auth/admin 路由。
- v3 §28.1 当前 **37** 条（R1–R37）。实施时除 R23/R26–R34 外还必须读：**R35**（native migration 独立账本与并发）、**R36**（PG18 的 NOT NULL 约束表示不能冒充 PG17 oracle）、**R37**（闸门不得继承操作者日志过滤或 GNU/BSD 输出差异）。
