...
| 路径 | 内容 | 状态 |
| --- | --- | --- |
| `MANIFEST.yaml` | 本目录的 parity ledger。每条 fixture 一行，`test_id` 前缀 `T-FIX`。 | 49 条：27 done / 22 todo |
...
| `computer/*.json` | Browser residency budget与CDP输入pure-plan的closed evidence boundary。 | done（T-FIX-0048..0049；真实engine/CDP仍按各自todo） |
...
```bash
python3 -c "import yaml,io;d=yaml.safe_load(io.open('fixtures/MANIFEST.yaml',encoding='utf-8'));print(len(d['entries']))"          # 49
...
python3 -c "import yaml,io;d=yaml.safe_load(io.open('fixtures/MANIFEST.yaml',encoding='utf-8'));print(sum(1 for e in d['entries'] if e['status']=='done'))"  # 27
```
