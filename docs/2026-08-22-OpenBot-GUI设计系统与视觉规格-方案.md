...
### 15.1 当前实施勾选（截至 2026-08-30，Batch 15–77；条目内历史计数按注明批次）
...
- [x] Batch72–77 Desktop structured realtime、window lifecycle与Local authority source代码闭环：host-owned closed wire与真实Tauri Channel、
  ...
  普通Web仍走原WebSocket/EventSource且有Batch74 release浏览器回归；Batch75–77无视觉/CSS/locale变化，
  MockRuntime与macOS文件证据不冒充真实native runtime/Windows ACL；
- [ ] Desktop Local authority的PG principal/package membership与Tauri setup接线、actual `tauri.conf.json`/
