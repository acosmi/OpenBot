...
### 15.1 当前实施勾选（截至 2026-08-30，Batch 15–81；条目内历史计数按注明批次）
...
- [x] Batch72–81 Desktop structured realtime、window lifecycle与Local authority/PG bootstrap代码闭环：host-owned closed wire与真实Tauri Channel、
  ...
  普通Web仍走原WebSocket/EventSource且有Batch74 release浏览器回归；Batch75–81无视觉/CSS/locale变化，
  MockRuntime与macOS文件证据不冒充真实native runtime/Windows ACL；
- [ ] Desktop Local sidecar binary build-sign、process/ready/shutdown/Windows key-store真机及真实Tauri `app_data_dir()` setup接线、actual `tauri.conf.json`/
