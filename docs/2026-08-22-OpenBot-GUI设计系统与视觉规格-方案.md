...
### 15.1 当前实施勾选（截至 2026-08-30，Batch 15–82；条目内历史计数按注明批次）
...
- [x] Batch72–82 Desktop structured realtime、window lifecycle与Local authority/PG lifecycle代码闭环：host-owned closed wire与真实Tauri Channel、
  ...
  普通Web仍走原WebSocket/EventSource且有Batch74 release浏览器回归；Batch75–82无视觉/CSS/locale变化，
  MockRuntime与macOS文件证据不冒充真实native runtime/Windows ACL；
- [ ] Desktop Local sidecar binary build-sign、Windows process/key-store真机、Batch79 schema/package组合及真实Tauri `app_data_dir()` setup接线、actual `tauri.conf.json`/
