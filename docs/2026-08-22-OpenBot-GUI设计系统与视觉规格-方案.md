...
### 15.1 当前实施勾选（截至 2026-08-30，Batch 15–76；条目内历史计数按注明批次）
...
- [x] Batch72–76 Desktop structured realtime与window lifecycle primitive代码闭环：host-owned closed wire与真实Tauri Channel、
  ...
  ApplicationService前拒绝，window close取消pending且不影响另一window。Batch76再以verified Rust authority
  先bind、actual `WebviewWindowBuilder` create/rollback、closed navigation/new-window/download与仅`Destroyed`
  unbind；macOS native scheme与Windows HTTP workaround分别闭集。普通Web仍走原WebSocket/EventSource且有
  Batch74 release浏览器回归；Batch75/76无视觉/CSS/locale变化，MockRuntime不冒充真实native runtime；
- [ ] actual `tauri.conf.json`/capability/binary与verified-session source、真实macOS/Windows Webview runtime、
