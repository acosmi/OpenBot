...
- [ ] **G2**：整关未通过；以下子项已经有本机机械证据：
  ...
  - [ ] 独立 SAML/XSW 与整体安全外审、Server KMS/HSM、Windows 原生构建；Desktop Local installed-app OAuth、browser/file/shell 与可点击 approval GUI 等 G4 余面仍缺。
...
当前总台账：parity **432/1669 done（1237 todo）**，fixtures **14/36 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
