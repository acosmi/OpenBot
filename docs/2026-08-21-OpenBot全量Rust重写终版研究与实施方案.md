...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] Admin第一批真实route：global AppSidebar只在admin probe成功时显示Admin section link；AdminShell先过同一gate，200px secondary nav只列Overview/Audit/Playground；Admin Home无断链，Audit经既有typed API按时间50条keyset分页并验证bounded object payload。release浏览器50→52、hard reload、landmark/current/overflow/console绿；真实PG session降为user后global link隐藏、统一NotFound且child不构造。只关闭三条route，完整AdminSidebar与formal golden仍todo；
...
当前总台账（`cargo xtask parity-check` 复算）：parity **693/1692 done（999 todo）**，fixtures **17/39 done（22 todo）**；v4 overlay carry/revalidate/split/superseded = **1674/16/2/0**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
