...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] tokens.toml 单源生成 CSS/Rust、Inter 4.1 随包、74 项 icon manifest/SVG 双向闭合；i18n en/zh-CN 637 叶键及占位符 exact；WASM gzip/CSS/fonts 与零内联脚本预算绿；
  ...
  - [x] Admin当前五条真实route：global AppSidebar只在admin probe成功时显示Admin section link；AdminShell先过同一gate，200px secondary nav只列Overview/Boundaries/Audit/People/Playground。Audit/People证据保持；Boundaries复用typed PolicyStore并补无隐式默认的首次preset、mode/deny/baseline管理与custom allow只读。release memory浏览器两preset、规则/mode/baseline/reload/中英/响应式全绿；真实PG keyed session + PolicyStore逐字段证明mode/deny/custom allow/updated_by，降当前admin后policy child零构造/请求。只关闭T-ROUTE-0004/0012/0013/0014/0020，完整AdminSidebar与formal golden仍todo；
...
当前总台账（`cargo xtask parity-check` 复算）：parity **700/1694 done（994 todo）**，fixtures **17/39 done（22 todo）**；v4 overlay carry/revalidate/split/superseded = **1595/97/2/0**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
