...
- [ ] **G2**：整关未通过；以下子项已经有本机机械证据：
  ...
  - [ ] 独立 SAML/XSW 与整体安全外审、Server KMS/HSM、Windows 原生构建；Desktop Local installed-app OAuth、browser/file/shell、approval真实 PG 浏览器竖切/完整thread集成等 G4/G6 余面仍缺。
...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] Approval 页面只展示服务端权威 effect/target/redacted arguments/change，count-only WebSocket hint只触发权威GET、连接/重连前GET、30秒fallback，fresh POST grant/deny；真实 Chromium 已验证批准后 card 消失+status与无1秒轮询，既有 APG ThemeToggle/LocaleSwitch、双视口与DOM审计保持；浏览器数据来自明确 test-only fixture，不冒充生产 PostgreSQL；
