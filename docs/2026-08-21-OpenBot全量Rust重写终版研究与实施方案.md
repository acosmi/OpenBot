...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] Approval 页面只展示服务端权威 effect/target/redacted arguments/change，count-only WebSocket hint只触发权威GET、连接/重连前GET、30秒fallback，fresh POST grant/deny；真实 Chromium 已验证批准后 card 消失+status与无1秒轮询，既有 APG ThemeToggle/LocaleSwitch、双视口与DOM审计保持；Batch57以testkit auth接真实production PostgreSQL approval coordinator并证明hard reload，不冒充production session-cookie浏览器；
