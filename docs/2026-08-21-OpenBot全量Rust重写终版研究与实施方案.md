...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] AppSidebar 的生产sign-out依赖已闭；同一bundle先读revocable，只有multi-user显示登出，UI helper只在Server回204后才允许导航；`/api/channels/events`/roster realtime仍未闭，故app-sidebar仍不勾；
...
当前总台账：parity **519/1673 done（1154 todo）**，fixtures **15/37 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
