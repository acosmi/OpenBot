...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] UI ledger 仅对已闭合的 EmptyState、Badge、ThemeToggle、LocaleSwitch 四条打勾；其余部分代码保持 todo；偏好 GET/PUT、native table/fixture 同批据机器证据 done；
  - [ ] reviewed 外部产品名/bundle id/deep-link 后的 `tauri.conf.json`/binary、真实 window lifecycle/multi-window integration、macOS arm64/Windows x64 原生发行构建，以及其余 23 个 primitive ledger 条目/45 业务组件/31 route、110 Web + 两平台各 54 golden、完整 axe/键盘 E2E 尚未闭合；
...
当前总台账：parity **440/1672 done（1232 todo）**，fixtures **15/37 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
