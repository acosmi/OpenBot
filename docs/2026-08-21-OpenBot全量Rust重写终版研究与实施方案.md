...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] tokens.toml 单源生成 CSS/Rust、Inter 4.1 随包、74 项 icon manifest/SVG 双向闭合；i18n en/zh-CN 342 叶键及占位符 exact；WASM gzip/CSS/fonts 与零内联脚本预算绿；
  ...
  - [x] UI ledger 只对有闭合机器证据的 26 条原语打勾：Batch18 前20条 + Dialog/Sheet + Menu + MessageScroller + Combobox/Select；其余源码仍保持 todo；
  ...
  - [ ] reviewed 外部产品名/bundle id/deep-link 后的 `tauri.conf.json`/binary、真实 window lifecycle/multi-window integration、macOS arm64/Windows x64 原生发行构建，以及其余 1 个 primitive ledger 条目/45 业务组件/31 route、110 Web + 两平台各 54 golden、完整 axe/键盘 E2E 尚未闭合；
...
当前总台账：parity **462/1672 done（1210 todo）**，fixtures **15/37 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
