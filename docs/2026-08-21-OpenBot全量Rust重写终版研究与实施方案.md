...
这条最初闭合的是**通用 application/infra/agent gateway 边界**。R71 后首个真实集合成员 `remember` 已以 production PostgreSQL effect 穿完：fresh ACL → metadata/schema → CEL → decision+attempt → capability CAS → `origin=remember_tool` memory → outcome+audit → durable tool pair → 下一次 sampling。Fact provenance、owner、Bot/thread target 都从当前 run/DB 取得；模型参数没有自报 ID 的位置。`AuthGeneration` 随不可序列化 execution scope 进入 executor，并在 memory INSERT 同一事务再次比较 generation/revoked/role，堵住 capability mint 后撤权竞态。R74 又让无 credential public HTTPS MCP read tool 经同一 `AuthorizedToolCall` 穿完；browser/file/shell、credential-backed MCP 与 Drive 仍须逐个实现，human proof-of-intent/approval GUI 也尚未闭合，所以本条仍不能写成“G4 整关通过”。（§28.1 R71 / R74）
...
### 24.1 实施状态勾选（2026-08-25；进度证据以机器台账为准）
...
- [ ] **G2**：整关未通过；以下子项已经有本机机械证据：
  ...
  - [x] W-5 batch 7 + R73/R74：Tenant Package 五 YAML、environment allowlist、theme/brand 替代裁决、PostgreSQL Agent/Profile/Channel 原子同步与 §6.5 membership；callback lifecycle 1 条与真 server-side-tools 5 条已闭合，G2 专项队列当前 **155 done / 79 todo / 234 total**；
  ...
  - [ ] 独立 SAML/XSW 与整体安全外审、Server KMS/HSM、Windows 原生构建；credential-backed MCP、Drive/browser/file/shell 与真实 approval 等 G4 余面仍缺。
...
- [ ] **G4**：整关未通过；以下 Rust built-in Agent 子面已有本机机械证据：
  ...
  - [ ] provider gate 要求的三家 recorded vendor trace 仍为 **0/3**，本批未使用 live vendor credential；human proof-of-intent/approval GUI、完整 run-wide token/cost/并发/computer budget、credential-backed MCP OAuth/bearer/outbound auth、run/user cancellation 向 RMCP notification 传播、MCP 专用 private egress/运行期 refresh/admin grant/effect UI、用户创建 remote Agent lifecycle/customer auth、interrupt/resume 与其余事件 durable/UI projection、Drive 及 browser/file/shell executor 尚未闭合。
...
当前总台账：parity **361/1660 done（1299 todo）**，fixtures **11/33 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
