...
>
> 文档状态：终版实施基线 v4（v3 + 2026-08-28 第三轮就地修订 R115–R125 + 2026-08-29 实施裁决 R126–R140，修订清单见 §28.1，修订方法与真源优先级见 §28.5；`docs/2026-08-28-OpenBot-TauriGUI-ElectronChromium-GrokBot大面积Rust迁移-v4修订计划-用户裁决版.md` 已被吸收，只作历史记录）
>
...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] tokens.toml 单源生成 CSS/Rust、Inter 4.1 随包、74 项 icon manifest/SVG 双向闭合；i18n en/zh-CN 677 叶键及占位符 exact；WASM gzip/CSS/fonts 与零内联脚本预算绿；
  ...
  - [x] Admin当前六条真实route：global AppSidebar只在admin probe成功时显示Admin section link；AdminShell先过同一gate，200px secondary nav只列Overview/Boundaries/Identity Providers/Audit/People/Playground。Audit/People/Boundaries证据保持；Identity Providers复用production keyed session、DynamicSsoService与v2 store，公开contract构造性不含secret/metadata/certificate，SAML/OIDC exclusive browser body、列表/注册/删除与取消清secret均闭合。真实PG SAML登记得v2 envelope且plaintext metadata0，删除与两audit唯一；降当前admin后IdP child零构造，HTTP list/register/remove计数不变。只关闭T-ROUTE-0004/0012/0013/0014/0019/0020，完整AdminSidebar与formal golden仍todo；
...
当前总台账（`cargo xtask parity-check` 复算）：parity **719/1694 done（975 todo）**，fixtures **17/39 done（22 todo）**；v4 overlay carry/revalidate/split/superseded = **1560/132/2/0**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
