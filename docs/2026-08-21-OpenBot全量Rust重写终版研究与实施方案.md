...
- [ ] **G5**：ComputerSecurityScope/runsc/fault injection/engine compromise 未完整实施。
  ...
  - [ ] **P1 整阶段仍未通过**：Windows Named Pipe peer PID+creationTime / Job Object / restricted token spike与 Ubuntu 24.04 x86_64 钉版 runsc 内 Chromium `Seccomp:2`、`NoNewPrivs:1`、layer-1 正向证据缺失；R63 禁止未授权派发 Actions，本机 macOS 不能伪造两平台结论。P2/P3/P4 未进入，G5A–G5F 仍不勾。
