...
这条闭合的是**通用 application/infra/agent gateway 边界**，不是 G4 的实际工具集合：browser/file/shell、MCP、Drive 与 provider executor 仍须在 G4 逐个实现并只能接收 `AuthorizedToolCall`。在至少一条真实外部 effect 走完录制 trace、故障注入与安全外审之前，不得把本条写成“G2/G4 整关通过”。
