...
>
> 文档状态：终版实施基线 v4（v3 + 2026-08-28 第三轮就地修订 R115–R125 + 2026-08-29–09-04 实施裁决 R126–R174，修订清单见 §28.1，修订方法与真源优先级见 §28.5；`docs/2026-08-28-OpenBot-TauriGUI-ElectronChromium-GrokBot大面积Rust迁移-v4修订计划-用户裁决版.md` 已被吸收，只作历史记录）
>
...
2. PostgreSQL、Chromium、Electron、操作系统 Keychain/KMS 作为外部引擎或系统设施存在；R129另允许Server runsc容器内的Xvfb只作Electron本地display backend，不作Screen UI/传输，不开放TCP/VNC。
...
| runsc | Batch55 已落只在 Ubuntu 24.04 x86_64 原生机运行的 `cargo xtask engine runsc-spike` 与容器内双 role probe；版本仍须该命令真实 PASS 后才钉入本表与 `tools/engine-pins.toml`，当前**没有 pin**。判据见 §24 G5 / R121/R128；在此之前 Server 生产 readiness 判据（§10.4）不变 |
| Xvfb（仅runsc内部display） | R129允许的Electron Linux本地display backend；固定`:99`、1280×800×24、`-nolisten tcp`，不是VNC/Screen UI。amd64 package version与`/usr/bin/Xvfb` SHA-256须随runsc spike PASS进入Server image provenance；当前**没有pin/实测值** |
...
| PostgreSQL | major 17；Desktop sidecar build source钉PGDG 17.11 exact tar.gz/SHA（R154）；上游 compose 用 `pgvector/pgvector:pg17`，Rust版不需要pgvector，Server平装`postgres:17`，§14.1 |
...
按 tenant/actor visibility 收窄并在 Repeatable Read 内完成。它不改变“两阶段 retrieval 尚未实现”
的结论，也不代表 Plugins/Skills GUI、legacy `/api/plugins/call` 或 G4/G6 整关完成（§28.1 R174）。
...
- run cost quota 使用 `run_cost_budget_unpriced` / `run_cost_budget_currency_mismatch` /
  `run_cost_budget_exceeded` stable terminal，并写 `agent.run_cost_budget_refused` hash-chain audit；
...
| `BOT_PRICE_CURRENCY` / `BOT_PRICE_MAX_INPUT_MICRO_UNITS_PER_MILLION_TOKENS` / `BOT_PRICE_MAX_OUTPUT_MICRO_UNITS_PER_MILLION_TOKENS` / `BOT_PRICE_SOURCE_URL` / `BOT_PRICE_SOURCE_SHA256` / `BOT_PRICE_OBSERVED_AT` | 新增 | R162；managed provider 六字段all-or-none，零默认/零联网抓价。currency恰3位uppercase且不做汇率换算；rate是operator对contract任一适用cache/uncached档位的maximum attestation；source只接受credential-free HTTPS并以document SHA+observed-at冻结。package provider用`model.yaml::pricing`同形字段。输出是保守cost upper bound，不冒充vendor最终账单 |
...
- 十个业务/parity 核心 crate + R127 唯一 Windows 安全边界 crate、toolchain、locked build；
...
- [x] **G1**：十个业务/parity 核心 crate + R127 唯一 Windows 安全边界 crate 的 locked build、Axum/in-process、28 表/13 migration/read checksum、tracing/metrics 四判据均已通过；R127 没有改变业务入口或 parity owner 域。
- [ ] **G2**：整关未通过；以下子项已经有本机机械证据：
  ...
  - [ ] 独立 SAML/XSW 与整体安全外审、Server KMS/HSM、Windows 原生构建；Desktop Local installed-app OAuth、browser/file/shell、approval完整thread/cancel/computer旅程等 G4/G6 余面仍缺。
...
- [ ] **G4**：整关未通过；以下 Rust built-in Agent 子面已有本机机械证据：
  ...
  - [x] Batch87 run-wide normalized token accounting：native 0024在`runs`上expand-only追加
    input/output/total aggregate、immutable derived output ceiling与exact last-sampling replay；Agent按
    sampling index跨tool loop累计，超限sampling先如实记账再以`run_token_budget_exceeded`收口。
    该derived ceiling只是现有per-sampling cap×最多9次sampling的机械一致性上界；没有冒充尚缺
    权威价格provenance与用户费用上限的完整cost budget；
  - [x] Batch88 operator-attested cost upper-bound accounting：package `model.yaml::pricing`或managed
    `BOT_PRICE_*`以provider/model/currency/maximum input-output rate/credential-free source URL+
    document SHA/observed-at形成immutable snapshot；native0025随run冻结并以whole micro-unit+
    millionth remainder跨sampling无浮点累计。缺snapshot保持NULL/unpriced，future observed-at在provider
    前与写入前双拒绝；该值是应对缺cache细分usage的保守上界，不是vendor账单；
  - [x] Batch89 actor-scoped per-run cost cap：closed GET/PUT 与 Settings 只按 AuthContext 三轴读写
  ...
  - [x] Batch91 process-wide concurrent tool runtime budget：host-only scheduling默认serial，只让build-owned
    11个ordinary compiled component显式parallel；human/remember/MCP/Drive/sandbox/unknown保持serial。
    stable waves只并行resource-lock不相交调用；跨run同key互斥，全run共享默认8、上限256的tool semaphore。
  ...
  - [x] approval production session-cookie browser：PG testkit模式seed production keyed-hash session并改用真实PostgresSessionAuthResolver；无cookie401，host-only HttpOnly/Lax testkit 303后GET/WS/fresh POST贯通，sessions/hashedSessions=1/1且hard reload保持；bootstrap不冒充OIDC/SAML登录；
  ...
  - [x] 用户创建 Agent lifecycle/customer Authorization backend：七条fresh same-origin HTTP写面只收closed
    form/path，Application从AuthContext铸`AgentAdministrationScope`并逐字段核权威receipt；PG事务锁定当前
    AuthGeneration/revocation、Agent/profile与package竞态，create/update/duplicate/hide/unhide/delete和
    ...
    T-UI-0031–0034；Desktop callback POST/DELETE也已接同一ApplicationService。Batch71又补channel
    list/detail/create/route与thread mint/status/begin/cancel/conversation九条unary framing；Batch72又落
    host-owned structured-event/Tauri Channel桥与window unbind/stale-binding fencing；Batch73再注册actual
    open/close command并把wire单源上收contracts；Batch74让三条WASM realtime owner按`isTauri`选择该桥并
    exact close；Batch75再把同一actual Webview全部internal stream的queued event-ref与live/in-flight
    ...
    Batch82把三verified program的version、stdin-only initdb、closed loopback/HBA、direct postgres child、TCP SCRAM ready与verified pg_ctl shutdown收成单一owner；
    Batch83再把同一child接fixed `openbot` database与Batch79 attestation→migration→principal→package，并以data-plane owner保持pool/child生命周期；
    Batch84再由持锁data-plane从共享OS generic-secret边界load/create per-instance Vault master，导出credential/audit/assertion/MCP OAuth材料；
    Batch85再把全部PostgreSQL application adapter wiring与唯一`Arc<dyn ApplicationService>`上收到无env/Axum/Tauri的Infra composition root，Server精确消费一次，Desktop data-plane可从持锁sidecar构造脱敏listener hand-off；
    Batch86从真实`AppHandle::path().app_data_dir()`以零窗setup顺序持有sidecar/data-plane/key/shared ApplicationService/BuiltInAgentRuntime/RunRelay，首窗最后创建、最后Destroyed后逆序停机；Desktop preference保持app-data文件。
    Windows full-runtime/key-store/process真机、Linux Secret Service、正式tauri.conf/capability/identity、可发布binary、真实Wry native runtime、
...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] data-backed `/agents` surface：固定上游`mine`与`!mine && public`两组、144×180 AgentCard、
    ...
    callback一次明文状态，并补Desktop callback POST/DELETE，关闭T-ROUTE-0007、T-UI-0031–0034；Batch71
    又接全部当前GUI所需channel/thread unary API，但没有把custom protocol普通body冒充SSE/WebSocket；Batch72
    新增host structured-event/Tauri Channel桥、closed wire、sequence/gap与window close/rebind fencing。
    Batch73又注册actual open/close command并上收共享wire；Batch74让roster/approval/thread三owner按
    ...
    Batch82补verified version→stdin initdb→direct child→TCP SCRAM ready→fast shutdown与unclean stale-lock fencing；
    Batch83补fixed business database、pre-write typestate attestation、shared bootstrap与clean data-plane shutdown；
    Batch84补per-instance OS-stored application master与credential/audit/assertion/MCP OAuth material；
    Batch85补无transport/window/env的共享PostgreSQL ApplicationService adapter assembly、Server单一consumer与Desktop脱敏listener hand-off；
    Batch86补Tauri app-data零窗setup、Desktop local preference adapter、BuiltInAgentRuntime/RunRelay、deferred protocol、首窗最后与最后窗逆序停机。
    T-UI-0126 formal golden、sidecar binary build/sign、Windows full-runtime/process/key-store真机/Linux Secret Service、正式tauri.conf/capability/identity、Desktop Remote session、真实Wry
  ...
  - [x] 根`/` Home Composer真实首发route：固定上游Explore/fallback顺序、结构化`@`选择与ARIA
    combobox；显式选择零模型且audit失败不推翻用户选择，无mention走production routing，失败回固定
    fallback。与`/channel/new`共用create→BeginRun executor并锁recipient/run identity；release浏览器四分支、
    reload/Explore零create、中英与600/1280全绿，真实PG create/begin/routing三条保持。只关闭
    T-ROUTE-0006；formal Home golden与跨页面完整Composer仍todo；
  - [x] authenticated guard + `/sign`：只有`/api/me` 401进入sign；其它失败显式可重试且protected child
    零构造。capabilities把环境provider闭成Google/Microsoft/Okta，企业SSO只投影布尔值并走email
    route ticket→continue；PG keyed session真实mount后/sign回/，Desktop custom protocol复用typed
    GetCurrentUser。只关闭T-ROUTE-0002/0031与五条对应测试；官方Okta SVG、live环境OIDC成功外跳和formal
    sign golden仍todo；
  - [x] root/app pathless layout：具名RootLayout在auth外唯一包住sign与全部route，host首帧theme/locale与
    per-Tooltip compound provider替代上游两React provider；具名AppLayout只在auth内构造并拥有唯一
    SidebarProvider/AppSidebar/topbar/main。release浏览器在1280×800与最小1024×640证明一屏外壳、pane
    内滚、root/app/main/nav唯一且外层overflow0。只关闭T-ROUTE-0001/0003；AppSidebar总项、formal
    shell/sign golden与Tauri window仍todo；
  - [x] `/bot?agent=` Direct Bot route：Agent只来自Server roster/detail，无query使用deployment-backed
    roster稳定首行且零硬编码ID；每Agent namespaced localStorage只接受有界UUID，known=true保持、false才
    fresh、检查失败保留并显式告警。fresh Server-minted ID在首run前不请求snapshot/SSE，begin成功后立即
    回到普通fail-closed规则；New chat只在mint成功后换thread。DirectBot与Channel复用唯一
    ConversationSurface，只切typed anchor。release浏览器证明首发、hard reload、New chat、双Agent key隔离与
    切回恢复，中英/1024×640/600×900全绿；PG thread begin/conversation/status五条绿。只关闭
    T-ROUTE-0008与T-TEST-0173–0179；formal Bot golden T-UI-0127仍todo；
  - [x] Composer纯状态地基：固定上游draft10+queue16逐条Rust移植；Segment→draft、single Agent、
    ...
    steer仍未落；Home只闭合首消息mention/routing，不把跨页面完整Composer业务项勾掉；
...
当前总台账（`cargo xtask parity-check` 复算）：parity **847/1710 done（863 todo）**，fixtures **23/45 done（22 todo）**；v4 overlay carry/revalidate/split/superseded = **1299/403/2/6**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
...
3. 非 Rust 例外仅为最小 engine shim（两 role，clean-room，§11.3）、外部引擎/服务、R129的runsc内部Xvfb display backend和受隔离的用户脚本数据；工作区零 npm（R117）。
...
| R126 | §10.3 / §10.6 / §11.2–§11.3 / §16.2–§16.3 / §19.1 P1 / §24 G5A、G5E（2026-08-29 Batch53：macOS Engine 最小闭环实施裁决） | R117–R119 规定 clean-room shim、ASAR/fuses/integrity、stdin+peer credential、macOS Enforced 与双 role conformance，但未钉 protocol/release epoch、Electron 43 第九 fuse、未裁决 cookie encryption，也没说明外层 Seatbelt 如何与 Chromium renderer sandbox 共存；最终品牌仍待 G8 商标清查 | 直接在 ES module 顶层 `await app.whenReady()` 会让 bootstrap 永久等 ready；Rust split UDS 后丢 frame 写半端会让 Node `allowHalfOpen=false` 自动关闭 ingress；外层 SBPL 被 renderer 继承会与 Chromium 自身 Seatbelt 叠加并 abort。若为跑通而删 `app.enableSandbox`/加 `--no-sandbox`，正好违反 R119/R121；若此刻猜最终品牌或开启 cookie encryption，又会把 P1 fixture 与 profile 迁移变成未裁决产品变化 | protocol/release epoch 固定 v1/1，由 contracts JSON 单源生成 shim module+hash；Electron 43 v1 九 fuse 严格为 `000011001`（RunAsNode/NodeOptions/inspect 关，embedded integrity/OnlyLoadAppFromAsar 开，cookie encryption 保持关闭，WasmTrapHandlers 保持开启，wire 增长即红）。macOS main executable 以精确 literal 继承 Rust SBPL；只有四个 Electron Helper + crashpad 五个精确 literal 允许 `(with no-sandbox)` 解除父 profile，renderer 随后必须由 `app.enableSandbox()`/`sandbox=true` 进入 Chromium OS sandbox且 `ProcessMetric.sandboxed=true`；Squirrel/ShipIt 与其它 bundle executable 不在 allowlist。外层 app/executable/bundle ID 使用明确 fixture identity，helper/framework 内部名保留，最终品牌仍归 G8。P1 macOS 子证据可落档，但 Windows/runsc spike 未齐前 P1 不勾、P2 不进 | `cargo xtask engine bundle|verify` 绿；shim 3 文件/404 LOC；contracts/computer/xtask=`88/18/90`，host双role=`2/0/0`；Browser/Component 各得1280×800 JPEG、renderer PID+creationTime+sandboxed，主/全部后代 TCP LISTEN=0，shutdown 后 PID/profile lock=0；frame malformed/stale/scope/generation/sequence 单测；`parity-check` 0 违反。官方依据：Electron ASAR format、fuses v1 config（含 WasmTrapHandlers=8）、ASAR Integrity、ProcessMetric.sandboxed/creationTime；详见 Batch53 文档 |
| R127 | §5.1 / §10.3 / §11.2–§11.3 / §16.3 / §19.1 P1 / §24 G1、G5A、G5E（2026-08-29 Batch54：Windows Engine 边界与 PE 探针） | R119 要求 Windows Named Pipe peer PID+creationTime、restricted token、Job/ACL，R117 要求 Windows PE ASAR Integrity；实现仍是 `cfg(not(unix))` 无条件拒绝。十个核心 crate 又统一 `unsafe_code=deny`，安全 std API 无法表达这些 Win32 原语 | 把 FFI 塞进 `openbot-computer` 会打破核心 unsafe 红线；先普通 spawn 再加 Job 有后代逃逸竞态；审计过的候选 `sandboxrs-windows 0.1.1` 会递归尝试给 `SystemRoot/ProgramFiles` 写 AppContainer ACL且删除 profile 不回收这些 ACL，不可接受。反向若只为交叉编译变绿就标 Windows 完成，会把从未运行的 Electron/ACL/Job/PE 结果冒充真机证据 | 第十一个 `openbot-windows-sandbox` 同时命中 §5.1 “独立安全边界/不同 feature graph”，是唯一 `#![allow(unsafe_code)]` crate，owner=`openbot-computer::engine`，公开 API 不暴露 raw handle/pointer；child 以 `CreateRestrictedToken(DISABLE_MAX_PRIVILEGE|LUA_TOKEN|WRITE_RESTRICTED)` + Restricted Code restricting SID创建并强制保持 medium integrity，token default DACL、profile/temp与两条 Pipe均加入该 SID（Pipe仍需当前 user normal check），因此main对未授权对象的写访问失败而renderer low-integrity正向信号不被main伪造；profile/temp另有 inheritable low label。`STARTUPINFOEX` 只继承 stdin+null stdout/stderr 三 handle，预建 Job 以 attribute list 在 suspended child 启动时附着，32 processes/4 GiB/kill-on-close、零 breakaway，再 resume；Rust逐 pipe校 `GetNamedPipeClientProcessId` 与 `GetProcessTimes` 100ns FILETIME，按Chromium先截微秒再转浮点epoch-ms的源码算法核对ready，renderer还必须属于同Job。Windows bundle将exe重命名为明确fixture，并用Win32 transaction写入、datafile模式重读official JSON的`Integrity/ElectronAsar` resource；Linux/runsc不在本裁决中。fidelity仍是`Degraded`，不冒充网络/可执行路径allowlist或同UID防护；Windows runtime未跑前P1仍红 | `cargo check` 与 `cargo clippy -D warnings` 对 `x86_64-pc-windows-msvc` 的 boundary/computer/xtask 全绿；本机 boundary pure tests `2/0/0`、computer `18/0/2 ignored`、xtask `91/0/0`；macOS bundle/verify及双 role host conformance回归 `2/0/0`；`parity-check` 0 违反。Windows 三 runtime test（Pipe/PE/ACL negative）与双 role conformance只编译未运行，故本行不提供 Windows pass 结论。官方依据：Microsoft `CreateRestrictedToken` / `CreateProcessAsUser` / `GetProcessTimes` / Job / Named Pipe 文档，Electron `ProcessMetric`/ASAR Integrity与Chromium `Time::FromFileTime`；详见Batch54文档 |
| R128 | §1.2 runsc / §10.4 / §11.3 / §16.2–§16.3 / §19.1 P1 / §24 G5A、G5E（2026-08-29 Batch55：runsc 可执行证据闭包） | R121 只写“Ubuntu 24.04 x86_64实测后钉版本”与三个renderer判据，没有可执行命令、pin时序、完整release形态、layer-1机械定义或负对照。Electron `ProcessMetric.sandboxed` 在Linux不存在；2026-07后gVisor官方release又由单runsc转为含sidecar目录的tarball | 先把最新日期写进pins是猜测；只校runsc单文件会漏sidecar漂移/自动补下载；`runsc do`官方说明默认可读整个host filesystem且rootless不支持常规`create`，不能冒充production OCI；只看renderer `NoNewPrivs:1`会被OCI init全局继承造成恒真，只看`Seccomp:2`也缺非renderer负对照；在shim里把Linux `sandboxed`硬写true则是renderer自报 | 新增`engine-runsc-probe`（`runsc-spike` feature保护的Cargo example，结构上不属于产品bins）作为runsc容器init，复用真实Engine bundle/digest、双UDS peer、hello/ready/frame/shutdown与Browser/Component两个role；`RunscAttestation`只在Ubuntu24.04 x86_64且`--gvisor-marker-file`生成的`/proc/gvisor/kernel_is_gvisor`存在时允许Linux直接spawn。shim在Linux明确发送`renderer_sandboxed=false`，Rust不接受自报；probe读取`/proc`并要求main `Seccomp!=2`负对照、renderer `Seccomp:2`/`NoNewPrivs:1`，且renderer相对main满足(pid+network namespace均不同)或user namespace不同；再验1280×800 JPEG、process tree TCP LISTEN=0、shutdown orphan/profile-lock=0。外层`cargo xtask engine runsc-spike`只在root Ubuntu24.04 x86_64运行，先校完整gVisor tarball SHA-256、安全路径、exact `runsc --version`、恰一非空sidecar目录；强制systrap/network none/host UDS+FIFO none/sidecar release ALWAYS + usage STRICT/gVisor marker、readonly root+bind、零capability、`noNewPrivileges`、64 pid/6GiB。只有报告全绿才打印pin candidate；它不自动改pins，版本/URL/hash必须与同轮真实证据手工入库。当前无Linux机器，故runsc仍无pin、P1仍红 | 本机`openbot-testkit` xtask=`93/0/0`；`x86_64-unknown-linux-gnu`对probe与xtask两条Clippy `-D warnings`绿；macOS `engine bundle|verify`与双role回归`2/0/0`；macOS调用runsc-spike在读artifact前确定性拒绝“requires native Linux x86_64”。未运行runsc、未产生`Seccomp/NoNewPrivs/layer-1`结果，不能记PASS。官方依据：gVisor OCI/rootless/install/flags文档（含2026-07 multi-file release）、Chromium Linux sandbox layer-1/2文档；详见Batch55文档 |
| R129 | §0.1条2 / §2.3条14 / §10.4 / §11.3 / §16.2–§16.3 / §19.1 P1 / §24 G5E（2026-08-29 Batch55：Linux Electron display backend） | R128 OCI没有DISPLAY；Electron官方长期明确hidden `BrowserWindow`在Linux仍需图形环境，真实headless Electron仍是未完成feature request。v4同时只列PostgreSQL/Chromium/Electron/Keychain为外部设施并禁止VNC Screen UI，故不能静默加入Xvfb或猜`--headless`/Ozone flag | 不加display使probe必然在sandbox判据前启动失败；使用未承诺的headless flag是假完成；把Xvfb连到TCP/VNC会新增远程画面面、违反§2.3。Xvfb又是非Rust可执行物，若不记录package/version/hash会绕过供应链真源 | 允许Xvfb作为**仅Server runsc容器内**的Electron本地display backend：不是Screen UI，不装/起x11vnc/VNC，不作host display。probe PID1先校`/usr/bin/Xvfb`等于外层Rust计算的SHA-256，再以固定`:99 -screen 0 1280x800x24 -nolisten tcp -noreset -ac`启动并等Unix socket；`EngineLaunchConfig::with_runsc_probe_display()`只设置固定`:99`，不接受free DISPLAY且移除XAUTHORITY。两role期间Xvfb PID是唯一允许background；单独要求其TCP LISTEN=0，末尾kill/wait后容器额外PID=0。rootfs `dpkg`中的xvfb amd64 package version与binary hash随PASS一起打印，必须进入Server image provenance/pin后才可称P1证据。`-ac`只在network-none的单scope runsc内部开放本地X socket；它不抵抗同sandbox renderer攻击X server，但即使Xvfb/main被攻破仍不得越过runsc scope，故G5E还要靠outer runsc而非把Xvfb当边界 | Electron官方issue/maintainer说明Linux hidden BrowserWindow仍需X server并以Xvfb为路径；X.Org manual定义`-nolisten tcp`。本机Linux target probe Clippy绿、macOS双role回归绿；未运行Xvfb/runsc，package version/hash尚无值，P1仍红。详见Batch55文档 |
| R130 | §6.2条10 / §8.5 / §13.4 / §17.2条3、9 / §19.1既有余项 / §24 G4、G6（2026-08-29 Batch56：Approval critical realtime） | R77/Batch14已有durable acting approval与跨replica waiter，Batch15已有可点击Leptos卡片，但页面每秒GET；§13.4又要求approval不可静默丢且Desktop队列满须显式断开。若照§7.5为approval另建cursor journal，会把已有`tool_approvals`复制成第二状态真源；若只发approval正文会扩大id/tool/target/summary泄露面。真实浏览器还暴露卡片成功后自删除owner会取消同一异步任务尾部，status与refresh可丢 | “无cursor hint”若不在连接/重连前GET会漏断线窗口；只比较count会漏“旧项解决+新项创建仍为1”；旧AuthGeneration socket若只返回空列表而不终止，仍违反现有subscription立即失效。相反继续1秒HTTP轮询既不是真realtime又持续放大DB查询 | 新增closed `ToolApprovalActivityEvent{pendingCount}`与ApplicationService subscription；socket没有approval id/tool/target/summary且从不直接改UI，只触发authoritative GET。PG同进程Notify或1秒poll后先重验user generation/role/revoke，再整页重读并比较完整projection；撤权/依赖错误发stable code后terminal，新handshake在headers前fail-closed。Axum要求trusted Origin+exact `openbot.tool-approvals.v1`，client data 1008；Desktop归Critical。WASM连接/重连前GET、30秒fallback、checked request epoch拒stale response；decision/refresh worker固定到ApprovalPage owner。该hint不是durable run event，不冒充cursor/replay | implementation `971f28af086cee5c5205623663fdd882505fadf1`。PG17.11 host SCRAM `1/0/0`实得owner/other隔离、跨replica0→1→0、generation撤权stable error→EOF且旧auth新订阅拒绝；Server approval HTTP/WS=`4/0/0`；六crate核心unit=`88/150/307/213/133/80`，Clippy/WASM/fmt绿。release浏览器fresh tab实得subscription1、旧1秒poll消失、card1→0、status保留、console0；bundle预算绿。API新增T-API-0170、event新增T-EVT-0087；parity=`695/999/1694`、overlay=`1600/92/2/0`、strict=`159/0/0`。真实PG浏览器单一竖切与完整thread集成仍todo，G4/G6整关不勾；详见Batch56文档 |
| R131 | §6.2条10 / §8.5 / §13.4 / §15.3 / §19.1既有余项 / §24 G4、G6（2026-08-29 Batch57：Approval真实PG浏览器） | R130把production PG realtime与release浏览器分别证明，但浏览器仍消费Mutex fixture；把两段证据并列不能证明click穿过native0020 state/audit/waiter。若为验收另加production endpoint或登录旁路会扩大产品面；若test fixture可对任意DATABASE_URL施加baseline/native又可能误写用户库 | 只看card消失可由前端本地删除伪造；只看granted row会漏waiter未醒、summary未清或audit缺失；test proof若放非API路径，未挂载时会被SPA fallback伪装200。auth固定身份又不能冒充production session-cookie矩阵 | 扩展既有`required-features=testkit` UI fixture的可选PG模式：环境变量只接受host=`127.0.0.1`且dbname前缀`openbot_ui_approval_fixture_`/PG标识符边界，默认memory不连库。专用空库施加正式baseline/native，事务seed user/roles/Agent/thread/member/run/lease，production coordinator启动真实`request_and_wait`；同一ApplicationService/Axum/WebSocket/GET/POST与release WASM完成点击。仅PG模式挂`/api/__fixture/approval-pg-proof`，只投影waiter闭集和六个计数；memory模式该API机械404。fixed auth的scope与PG逐字段相等但仍明确是testkit，不扩成登录结论 | implementation `2cecfeab7e225cb0ad3bd4ae134c26193aa0c30b`。最终独立PG17.11 host SCRAM专用库：初始`approvals/pending/requestedAudits/waiter=1/1/1/waiting`；浏览器权威target+redacted card后click grant，终态`granted/pending/summariesCleared/requestedAudits/grantedAudits/waiter=1/0/1/1/1/granted`，status保留、console0、hard reload card0；server打印waiter granted，实例/文件全清。fixture unit=`1/0/0`、bin Clippy/fmt、parity0违反；无新产品API/T-ID。production session-cookie浏览器与完整thread/cancel/computer旅程仍todo，G4/G6整关不勾；详见Batch57文档 |
| R132 | §6.2条6–10 / §8.5 / §13.4 / §15.3 / §24 G2、G4、G6（2026-08-29 Batch58：Approval production session-cookie browser） | R131的浏览器仍由FixedAuthResolver直接放行；它能证明PG approval state，却不能证明cookie→keyed hash session→current generation/role/revoke/freshness→Origin WebSocket/POST。直接让浏览器脚本写cookie既无法验证HttpOnly，也会绕开Server cookie形状；为测试接真实OIDC会把外部IdP/credential引入本批 | test-only raw token若以明文入DB会把旧Better Auth兼容形态冒充Rust session；bootstrap若进入product router就是认证旁路；若cookie不经fresh session，grant POST会被正确拒绝。另一方面浏览器控制面不应读取/修改cookie store，必须由HTTP响应设置 | PG fixture seed用production `SessionTokenHash`只存`sh1_` keyed hash，并构造session/user generation、role、fresh timestamps；PG模式改用真实`PostgresSessionAuthResolver`，memory仍Fixed。仅testkit PG模式挂无输入303 `/api/__fixture/session/start`，返回host-only `Path=/; HttpOnly; SameSite=Lax`+no-store并跳`/approvals`；无cookie先401，浏览器只能经该响应取得cookie。proof增加sessions/hashedSessions计数；默认memory两test API均404。该bootstrap不是OIDC/SAML登录证据且不进production binary | implementation `1fdc65c4261c9dfa4aaf4f5df8ce58b94a24630a`。最终PG17.11 host SCRAM专用库：curl无cookieapproval=401/bootstrap=303；浏览器bootstrap前card0+load error，后card1+权威target/redacted；grant终态沿用R131且`sessions/hashedSessions=1/1`，status/console/hard reload全绿，waiter granted；实例/文件全清。fixture unit=`2/0/0`、bin Clippy/fmt、parity0违反；无新产品API/T-ID。真实OIDC/SAML登录与完整thread/cancel/computer旅程仍todo，G2/G4/G6整关不勾；详见Batch58文档 |
| R133 | GUI真源 §5.1–§6.2 / §3.1条8 / §3.2 / §5.2 / §8.6 / §13.1–§13.3 / §24 G2、G6（2026-08-29 Batch59：Admin shell + Audit） | production admin status/audit API与PG reader早已闭合，但GUI只有`/admin/playground`直挂在AppShell，既无admin gate/secondary shell，也无audit消费面。若一次画全上游AdminSidebar会制造People/Plugins等断链；若child在gate前构造，非admin仍会发audit业务请求；若把fixture浏览器当PG reader或把功能截图当formal golden会混淆证据层 | renderer只靠`CurrentUser.role`可被stale DTO误导；audit cursor/payload若不bounded/unique会造成重复、DOM/内存膨胀或把结构化facts当HTML。v3 §3.2又要求非admin统一不可见，不能重演上游redirect到首页泄露route存在 | 新增AdminShell先GET既有`/api/admin/status`，通过前不构造ChildrenFn；global AppSidebar同样只按该probe显示Admin入口；401/403/404统一NotFound，其它失败显式alert。200px secondary nav只列Overview/Audit/Playground三个真实destination；Admin Home只列Audit/Approvals/Playground。Audit helper固定limit50+opaque cursor、same-origin/no-store，验证页长/ID唯一/字段与64KiB object payload；跨页duplicate拒绝。fixture给52条固定脱敏事实用于GUI分页；生产reader不变。完整AdminSidebar与page golden分开保持todo | implementation `f071fc807a8a886a97d8cc45670de7e9af6fe097`。UI=`136/0/0`、Server lib=`213/0/0`、fixture=`2/0/0`，Clippy/WASM/fmt绿；release浏览器global Admin link1+section current、admin h1/nav2/current1、audit50→52/duplicate0/payload展开/hard reload/overflow0/console0，Playground同shell。真实PG session把admin原子降user并同步generation后global Admin link0，`/admin`与`/admin/audit`均NotFound、nav0/child0。只关闭T-ROUTE-0004/0012/0013；T-UI-0028与T-UI-0132/0139仍todo。routes=`11/21/32`、总parity=`698/996/1694`、overlay=`1597/95/2/0`；详见Batch59文档 |

| R134 | GUI真源 §5.1–§6.2 / §3.1条8 / §3.2 / §5.2–§5.3 / §6.2 / §13.1–§13.3 / §15.3 / §21.1条2 / §24 G2、G6（2026-08-29 Batch60：Admin People） | W-3a早已有production People list/role/access原子API、admin floor/self/last-admin与generation/audit规则，但GUI route仍404。固定上游要求250ms server-side search、keyset、role switch与remove/restore；只筛首屏会漏人，只用renderer角色会绕权威gate，把身份/id放mutation body又会扩大authority input。若为验收只用Mutex fixture，会把PG rule/audit/session竖切冒充完成 | list返回行若跨页重复会制造重复控制；search旧响应晚到可覆盖新查询；row signal若在异步worker owner内创建，worker结束即dispose。真实release浏览器确实在首次Load more后抓到WASM `unreachable`，证明host单测无法代替owner旅程。配置管理员和本人只能由Server事实禁改，GUI禁用只是避免提供必拒操作。非admin必须在People child构造前统一NotFound且零People请求 | shared contracts固定`ChangePersonRole{role}`/`ChangePersonAccess{revoked}`与authoritative `PersonResponse`；UI helper固定limit50、same-origin/no-store、opaque cursor、页内/跨页unique与receipt id/result；250ms debounce配checked search generation/request epoch。loader与row signals固定到PeoplePage stable Owner；自有静态row避免把Button/Switch嵌进交互Item。Admin Home/shell只在route闭合后加入People。memory fixture给52行/页外target/self/configured/revoked状态；PG fixture在既有loopback+prefixed guard内复用production session resolver与PostgresPeopleAdministration | implementation `aeb38f90b43b1796fd961fc02a3a10cb1b71f359`。contracts/UI/Server lib/fixture=`88/139/213/3`且0失败；三crateClippy/WASM/fmt绿。release memory浏览器50→52、search唯一、双向role/access、hard reload、中英、600/1280、runtime error0；PG17.11 SCRAM双向后`generation/role/revoked/targetSessions/roleAudits/revoke/restore/actorHashedSession=3/user/0/0/2/1/1/1`。当前actor降user并同步generation后新tab仅status403×2、People API0、global/admin nav/child0；两条403是预期网络事件，无runtime error。bundle=`1498159/100390/740216/1/0`；只关闭T-ROUTE-0020，T-UI-0028/0140仍todo。routes=`12/20/32`、parity=`699/995/1694`、overlay=`1596/96/2/0`、strict=`159/0/0`；详见Batch60文档 |

| R135 | §3.1条8 / §3.2 / §5.2–§5.3 / §6.1–§6.2（GUI第一真源）/ §8.1–§8.3 / §13.1–§13.3 / §15.3 / §21.1条2 / §24 G2、G6（2026-08-29 Batch61：Admin Boundaries） | R57早已有production PolicyStore、typed GET/PUT、fresh admin/Origin与default-deny，但GUI route仍404。固定上游页假设GET永远返回default `allow:["true"]`；本仓§8.3明确推翻隐式allow并要求首次wizard由admin选preset，却没定义preset字节。若UI收到null就复制上游空白页，部署永远无法完成首次设置；若自动填allow true则直接违反第一真源。迁移来的custom allow又不能被二值UI静默覆盖 | CEL 4096-byte上限原只在domain，GUI若另写魔数会漂。规则按文本作For key时重复行会冲突；按值filter remove又会一次删全部重复。async save挂在会重渲染的行会重演owner取消receipt；真实浏览器还发现duplicate draft alert在成功移除后仍残留，已不对应current policy。非admin必须在child构造前统一NotFound且零policy请求。截图不等于formal golden | 把MAX_ACTION_POLICY_EXPRESSION_BYTES上收到wasm-safe contracts并由domain re-export；共享`ActionPolicyResponse{policy:Option<_>}`保留null。首次wizard无预选，两个exact preset均enforce+deny[]：严格=`allow[]`，固定上游兼容=`allow["true"]`；admin必须点击才PUT，之后两个baseline可显式互换。custom allow不是这两种时逐字只读、零覆盖按钮；mode/deny仍按固定上游，新增rule过ECMAScript trim/4096/duplicate，existing坏/超限rule仍可见并按index单删。load/save worker固定Page owner，只有exact authoritative receipt更新UI并清stale draft error。memory fixture从null开始，PG fixture用custom allow证明保留 | implementation `ae8e9c73f46b17d97c54380a29931feb612dd5ca`。contracts/domain policy/UI/Server lib/fixture=`88/67/143/213/4`；fixed-upstream policy/CEL/Application dry-run/Server policy/transport=`28/6/1/4/1`；六crateClippy/WASM/fmt绿。release memory浏览器两preset、mode双向、preset+Enter、duplicate、remove、baseline双向、reload、中英、600/1280、runtime error0；PG17.11 SCRAM无cookie401/303后custom allow只读，add password deny+dry-run落DB为`dry-run/[password]/[actor custom]/fixture-actor/1`并reload保持。降admin后status403×2、policy GET/PUT0、child0；预期403只作network事件。bundle=`1527597/103671/740216/1/0`；只关闭T-ROUTE-0014，T-UI-0028/0133仍todo。routes=`13/19/32`、parity=`700/994/1694`、overlay=`1595/97/2/0`、strict=`159/0/0`；详见Batch61文档 |
| R136 | §3.1条1、8 / §3.2 / §5.1–§5.3 / §6.1–§6.2（GUI第一真源）/ §6.2条9 / §13.1–§13.3 / §15.3 / §17.2条8 / §21.1条2 / §24 G2、G6（2026-08-29 Batch62：Admin Identity Providers） | W-7b早已有production deployment-owned SAML/OIDC store、动态register/update/list/delete、v2加密与fresh admin/Origin，但GUI route仍404。固定上游页只展示动态provider，环境Google/Microsoft/Okta省略；支持SAML/OIDC添加和按deployment删除，不暴露编辑。现有公开projection与response envelope却在infra，UI按§5.1只能依赖contracts。若把infra注册输入直接上收并derive Serialize/Clone，会把client secret从不可复制/不可回传边界降成普通DTO；若只在UI再写一份公开projection，则Server/UI无编译期join | OIDC成功注册必须拉真实discovery；为浏览器验收伪造本地成功会把fixture冒充协议证据。列表若不限制行数/ID/domain唯一会让不可信响应制造重复控制或内存膨胀；domain若只靠浏览器原生required会与Server规范化/重复规则分叉。异步成功后乐观插入又不能证明PG权威状态。非admin必须在child构造前拒绝且零IdP请求。临时表单需要持有secret，但取消/成功/unmount后不得残留；普通日志/Debug/response永远不能取得它 | 把`SsoProtocol`、`RegisteredIdentityProvider`、closed list/delete receipt与Server输入上限上收到wasm-safe contracts；公开projection类型构造性无secret/metadata/certificate。另建只Serialize、不Clone/Debug/Deserialize且字段私有的browser registration request，OIDC discovery只由issuer派生；infra继续用不可Clone/Serialize、Debug redacted的`SecretInput`反序列化同一wire。UI前置校provider ID、最多16个canonical ASCII domain、OIDC/SAML issuer、HTTPS endpoint与client/metadata字节上限；list再验256 cap、provider/domain唯一与canonical，register receipt绑定非secret请求字段，三写面成功后都权威refetch。page owner固定load/register/delete；取消清全部draft。PG testkit模式复用production keyed session与具体`DynamicSsoService`，只给SAML正向（无需伪造OIDC出网）；test-only探针只投影密文/审计计数与三面HTTP计数 | implementation `a90c0891b222822c403fbdbb8f3de11407abf139`。contracts/UI/Server lib/fixture/infra config=`90/147/213/5/4`且0失败；四crate all-target/all-feature Clippy `-D warnings`、UI WASM、fmt绿。release浏览器空提交5字段拒绝；PG17.11 SCRAM登记SAML后row=`SAML/acme.example,second.example/https://idp.example/entity`且`providers/saml/v2/plaintextMetadata/registerAudit=1/1/1/0/1`，hard reload保持；中英、OIDC branch/password secret、取消清secret、删除回空态后`providers/removeAudit=0/1`，runtime error0。降actor为user并同步user/session generation2后NotFound、child/button/dialog0，HTTP计数保持`list/register/remove=1/0/0`。1280 overflow0；Browser viewport能力未应用600 override，如实未跑，formal golden T-UI-0138仍todo。bundle=`1585631/105118/740216/1/0`；只关闭T-ROUTE-0019。routes=`14/18/32`、parity=`701/993/1694`、overlay=`1594/98/2/0`、strict=`159/0/0`；详见Batch62文档 |
| R137 | §3.1条2、4 / §3.2 / §5.1–§5.3 / §6.1–§6.2（GUI第一真源）/ §8.6 / §13.1–§13.3 / §15.1–§15.3 / §21.1条2 / §24 G3、G6（2026-08-29 Batch63：Home Composer） | R95早已闭合production channel create、create-time routing与native BeginRun，`/channel/new`也可首发；但根`/`仍临时渲染ApprovalPage，用户无法走固定上游Home的“写消息→显式@或自动routing→创建channel”主入口。固定上游fallback恰为`explore[0] ?? agents[0]`，explore恰`!mine && public`；显式mention仍调用route只为记audit且route失败被吞，不能让记录故障推翻用户选择。若直接复用`/channel/new`的必选收件人会删除自动routing；若把文本里的`@名字`重新解析成authority会把自由文本冒充structured Agent ID | Server inferred response是不可信数据，roster外ID/viaMention错位/超长reason不能直接创建channel；UI fallback若每次retry重新routing会让同一消息换recipient。Home另写create→begin会与`/channel/new`的unknown-create/begin-replay语义漂。Textarea旁只画listbox但不连ARIA会让可见mention chooser在AX树中断裂；旧mention若不移除会同时携两个structured recipient。异步navigate后owner卸载还可能取消尾部状态。把Home挂顶层`features/mod.rs`又会让overlay前缀空转命中全部features done target | 根route改为`shell::home::HomePage`；roster只接受有界可路由Agent，explore/fallback逐字固定上游。`@`仅在start/whitespace边界触发候选，按name/id过滤；Enter/click选择时存Server Agent ID并把旧mention从文本移除，手写`@`永不自行取得authority。Textarea只在opt-in时输出`role=combobox`+controls/expanded/autocomplete。route response校ID/name/500 code-point reason和explicit/inferred flags；reason上限从domain上收到contracts并由domain re-export。显式recipient无论route audit结果都保持；inferred坏/失败回UI fallback。`StartAttempt`/executor由既有`channels::new`拥有，Home复用同一create→BeginRun→navigate：route+run identity在首试固定，unknown create阻断，begin/navigation retry复用channel/run。testkit deterministic routing/proof只投影计数/ID/thread，零消息/模型reason | implementation `07bff3a2b0ab010f2d4f4d78702a61d03aa3d1d5`。contracts/domain routing/application routing/UI/Server lib/fixture=`90/12/6/152/213/6`；PG17.11 SCRAM create/native begin=`2/0/0`、routing/audit=`1/0/0`；五crate all-target/all-feature Clippy `-D warnings`、UI WASM/fmt绿。release浏览器：initial52；structured mention→Knowledge Desk得complete0/explicit1；无mention→Risk Analyst得complete1/inferred1；completion+audit失败得failedRecord1且fallback→Knowledge Desk；另显式mention audit失败得complete0/failedRecord1/recorded0仍按用户选择。前三条主矩阵channel52→55，均有thread/首消息/导航；reload与Explore click零create，中英、600/1280 overflow0、main1/nav1/h1 1、Explore2、duplicate/nested/alert/runtime error0。最终bundle又实跑Home mention与ChannelNew各建一条回归。bundle=`1605538/108118/740216/1/0`；关闭T-ROUTE-0006，revalidate T-ROUTE-0010/T-TEST-0873–0883/T-UI-0020/0030；T-UI-0043/0130仍todo。routes=`15/17/32`、parity=`702/992/1694`、overlay=`1579/113/2/0`、strict=`159/0/0`；详见Batch63文档 |
| R138 | §3.1条1 / §5.1–§5.3 / §6.1–§6.3 / GUI真源§4.6.3、§5.1–§6.2 / §13.1–§13.3 / §15.3 / §21.1条2 / §24 G2、G6（2026-08-29 Batch64：Sign-in + authenticated guard） | 环境/dynamic OIDC、SAML、keyed session与`/api/me`早已生产闭合，GUI却只按pathname把`/sign`画成静态“已退出”，其它route在任何session probe前直接构造AuthenticatedShell、偏好loader与Sidebar。capabilities响应又由Server私有自由String DTO承载，Desktop custom protocol没有`/api/me`；固定上游则要求`_authed.beforeLoad`、sign反向redirect、三provider与enterprise email routing | 把所有非200都当“未登录”会让503/坏response形成redirect loop；先构造protected child再异步跳转会发出未授权runtime/roster请求。只在首个mount判断又漏client-side `/sign` 导航。把动态enterprise provider/domain列进按钮会破坏匿名反枚举；把callback/远端错误正文交给GUI又扩大数据面。Input若在IME composition时把Enter提交会截断邮箱。品牌同页还有独立冲突：GUI真源要求四个官方SVG；本轮官方Okta press kit只有Okta PNG，不能描摹或拿第三方/上游currentColor标冒充 | wasm-safe contracts把环境provider闭成Google/Microsoft/Okta且canonical，动态SSO只投影布尔；统一start/route receipt。AuthenticatedBoundary只有`/api/me`401进入sign，其它失败可重试且child零构造；checked retry拒stale response，已认证exact `/sign`在mount与pathname观察两路replace `/`。偏好context移到边界内。SignIn capability-first，OIDC只接HTTPS绝对URL，enterprise email只在202后continue；Input email自动autocomplete且IME-safe Enter。Desktop `/api/me`复用host-bound AuthContext→GetCurrentUser。testkit auth journey只在required-feature binary挂；品牌零文件入仓、T-UI-0039保持todo | implementation `5c51626d2f588e2b52b5adcfbed99f05f1743e5a`。contracts/UI/Server lib/fixture/Desktop=`92/155/213/7/80`；四crateClippy与最终UI Clippy、release/offline/locked Trunk、fmt/i18n/design/CSS全绿。PG17.11 keyed session浏览器实得无cookie401、guard protected增量0、三provider、OIDC拒绝本地化、enterprise start/continue、sessions/hashedSessions=1/1、authenticated mount与/sign回/；最终干净计数`capabilities/OIDC/start/continue/me/protected=4/2/1/1/5/0`。最终逐字WASM另实得channel/new→sign、autocomplete email、enterprise continue、authenticated mount、logs0；synthetic client history控制层不可用，只记纯状态测试，不冒充浏览器。bundle=`1659274/109378/740216/1/0`；关闭T-ROUTE-0002/0031与T-TEST-0166/0168/0169/0171/0172，T-TEST-0167/0170、T-UI-0039/formal golden保持todo。routes=`17/15/32`、tests=`377/670/1047`、parity=`709/985/1694`、overlay=`1570/122/2/0`、strict=`159/0/0`；详见Batch64文档 |
| R139 | §3.1 末段5个layout / GUI真源§5.1–§5.3、§6.1 / §13.1–§13.3 / §21.1条2 / §24 G6（2026-08-29 Batch65：Root/App pathless layout） | R138后sign/auth与App shell可见行为已存在，但root/app结构仍散在`app.rs::App/AppShell/AuthenticatedShell`，台账target `openbot_ui::shell::layout::{root,app}`没有真实owner；root又没有固定上游`min-h-dvh w-full`的显式wrapper。若直接勾done，只能靠历史截图猜归属。上游root另有ThemeProvider/TooltipProvider，而本仓第一真源已经改了两者机制 | 把typed preference搬到auth外去“复刻ThemeProvider”会让未登录sign发protected请求，推翻R138；再造全局Tooltip状态会与已完成的每实例400ms/focus/Escape compound provider双写。App main若承接document滚动，会破坏Chat/secondary pane自己的scroll anchor；反向把所有main都overflow-hidden又会让普通List/Form无法滚。仅用class存在测试则不能证明数值 | 新建具名RootLayout与AppLayout。Root在Router内/AuthBoundary外，唯一包sign与全部route；host改写html class/lang承担首帧theme，per-Tooltip Provider承担交互，root只固定width100%/min-height100dvh。App只在auth成功后构造，原样拥有边界内preference、唯一SidebarProvider/AppSidebar/topbar/main；app height100dvh+overflow hidden，stage/main min-height0，main overflow auto。host单测直接拆CSS数值；release browser分别观察sign与authenticated DOM/尺寸 | implementation `d614a6213755a47ce4d5399ce0ae4af1b2124447`。UI=`156/0/0`、Clippy/WASM/fmt与release/offline/locked Trunk全绿；i18n/design/CSS=`689/98+74/335`，bundle=`1651268/109435/740216/1/0`。sign 1280×800=`root/app/main/nav 1/0/1/0`、root/body800/800、overflow0/log0；auth app 1280=`1/1/1/1`、app/main/body800/756/800、x0；1024×640=`1/1/1/1`、640/596/640、x0、duplicate/nested/log0。关闭T-ROUTE-0001/0003；routes=`19/13/32`、parity=`711/983/1694`、overlay=`1568/124/2/0`、strict=`159/0/0`；AppSidebar总项/formal golden/Tauri window仍todo；详见Batch65文档 |
| R140 | §3.1条5 / §3.2 / §5.1–§5.3 / §7.4 / GUI真源§5.1–§6.2 / §13.1–§13.3 / §15.1–§15.3 / §21.1条2 / §24 G3、G6（2026-08-29 Batch66：Direct Bot Chat） | native mint/status/begin/snapshot/SSE/cancel早已存在，`/bot`却仍404；固定上游route又把无query Agent硬编码为`risk-analyst`，其hook以每Agent localStorage保存thread、known=false才fresh、检查失败保留、New chat另mint。若直接复制CopilotKit Chat会重引Intelligence与JS runtime；若浏览器自行mint UUID会失去deployment指纹；若把任何status错误当false会因网络抖动丢历史。现有ChannelConversation又把anchor写死Channel，复制一份会让Stop/terminal/component边界漂 | Server mint只铸identity，首run前snapshot合理不存在；首版即刻GET造成预期404网络错误，若把404普遍解释为空又会让已持久thread被删除后静默重开。localStorage值与query Agent都是不可信输入，必须过typed path/Server visibility；Agent切换/New chat异步任务还可能互相覆盖。上游stopped-turn传raw Error/prose，与本仓closed terminal裁决冲突，不能复活 | 新建authenticated `BotChatPage`与真实Sidebar destination；query join roster/detail，无query取deployment-backed Server roster稳定首行，零硬编码ID。每Agent namespaced key只收有界UUID；known=true keep、false fresh、error keep+显式history unavailable；New chat解析期disabled且只在Server mint成功后swap。把ChannelConversation收为共享ConversationSurface，anchor参数只允许Channel/DirectBot typed enum。fresh minted thread在首begin前直接空状态且零snapshot/SSE请求，begin成功即撤例外，此后404 fail-closed；begin retry保持exact thread/run/Agent/message。testkit proof只投影计数/ID不含正文 | implementation `794c6ec5f2b42d5deccadae77cd285f54d9947e2`。UI=`164/0/0`、fixture=`8/0/0`，Server/UI Clippy、WASM/fmt与final release/offline/locked Trunk全绿；PG17.11 SCRAM thread begin/conversation/directory=`3+1+1/0/0`。release浏览器clean初始mint/status/conversation=`1/0/0`；首发runs/threads/persisted/messages=`1/1/1/2`；hard reload mint不增/status1/同thread两消息；New chat后mint2且旧thread仍persist，第二次发送后`2/2/2`；Risk Analyst选择生成第二key/mint3，切回Research Partner status2并恢复第二会话。中英、1024×640/600×900、overflow/duplicate/nested/alert/runtime error均0。bundle=`1683002/109859/740216/1/0`；关闭T-ROUTE-0008/T-TEST-0173–0179，T-UI-0127保持todo；routes=`20/12/32`、tests=`384/663/1047`、parity=`719/975/1694`、overlay=`1560/132/2/0`、strict=`159/0/0`；详见Batch66文档 |
| R141 | §3.3 / §5.2 / §6.1–§6.7 / §8.5–§8.6 / §13.1–§13.3 / §15.3 / §21.1条5 / §24 G4、G6（2026-08-29 Batch67：Admin Components + Playground） | 固定上游Components index/detail对compiled与sandboxed行共用generic draft/publication mutation；v4既已裁决sandbox source+description+revision原子发布且sandbox无data function，照搬会把同一能力拆成两个写面并新增未授权能力。本仓此前只有catalogue读面和Playground sandbox写面，Admin Components两route与六个API/十一条上游测试仍todo。 | browser写成功后若继续用本地草稿猜Server状态，会在并发/audit commit-unknown时显示假成功；Agent/function grant若不逐次校当前行与Agent可见性，会留下orphan或撤权不生效。sandboxed function grant会越过R113的零data-function边界。production CSP只有`default-src 'none'`却漏`frame-src`，会把同源runner也拦掉；Playground按name keyed会在同名revision更新后保留stale row。 | contracts新增closed governance mutation/receipt；Application重复admin角色、名称/Agent/function/description边界并逐字段核receipt。Axum与Tauri只暴露六条exact typed写面，fresh Origin在读body前，成功no-store；PG SERIALIZABLE + row lock把grant/revoke/draft/publish与allowlisted hash-chain audit同事务，publish提升draft，unpublish保留独立grant。compiled可全写；sandboxed在Components只允许Agent withholding，draft/publication只经Playground，function一律Conflict。UI只以权威receipt替换row、失败switch回滚；CSP只加`frame-src 'self'`，Playground keyed identity覆盖完整row digest。 | implementation `ecfaf09746bbaf131411b9a8d66fb2d1ecb6c1bc`。Contracts/Application/Agent/Infra/Server/fixture/Desktop/UI=`93/151/34/307/214/8/81/166`；PG17.11 SCRAM component catalogue=`2/0/0`，覆盖audit rollback、幂等created_at、publish提升draft、unpublish不清独立grant与sandbox拒绝。8-crate Clippy `-D warnings`、UI WASM、fmt/diff、Engine/tools/GUI gates全绿。release浏览器index16、compiled draft/publish/Agent/function双向、sandboxed禁用矩阵、Playground draft→published revision1/hard reload、中英、1024×640/600×900与DOM/runtime负面审计全绿。bundle=`1741699/112331/740216/1/0`；关闭T-API-0043–0048、T-ROUTE-0015/0016/0021、T-TEST-0437–0439/0443–0448/0454–0455；parity=`739/955/1694`、overlay=`1540/152/2/0`、strict=`159/0/0`。formal Components/Playground golden、Desktop独立sandbox renderer与sandbox MessageChannel正向执行仍todo；详见Batch67文档 |
...
| R145 | §3.1条4–5 / §4.1–§4.3 / §5.1–§5.3 / §13.1–§13.4 / §15.1–§15.3 / §21.1条2 / §24 G3、G4、G6（2026-08-30 Batch71：Desktop channel/thread unary transport） | R144后同一Leptos bundle在Desktop虽然能管理Agent，AppSidebar channel list、Home routing、`/channel/new` create→begin、`/channel/:id` detail/conversation以及Direct Bot mint/status/cancel仍全部落generic 404。只补create/begin会在成功导航后立刻404；把Web的EventSource URL也返回普通`Response<Vec<u8>>`又会伪造流式能力。 | WebView request body在进入adapter前已经materialize；首消息/route文本若在malformed、oversize、unknown API或wrong method路直接drop而不覆零，会留下不必要的host副本。自由query/path解析会让Desktop接受Server拒绝的authority输入。反向给这些`OriginAuthenticated`写面擅加fresh要求又会使同一bundle在Web成功、Desktop 401。channel/thread JSON总body必须与Server全局1MiB同量级，不能只按message字段上限忽略envelope。 | 在generic API fallback前显式接channel list/detail/create、route与thread mint/status/begin/cancel/conversation九条unary面；每条只构造既有AppCommand并经同一InProcessTransport。channel query只收唯一limit/cursor并按form-urlencoded percent/plus解码；path只收closed segment/shape，events/mint静态词保留。用户文本JSON总body取contracts 1MiB常量，成功/坏JSON/超限以及generic API/非GET fallback全部覆零原始Vec；Server这些写面是OriginAuthenticated，custom scheme天然同源，只要求host-bound window而不虚加fresh。status精确200/201 replay与200/202 cancel；events/ws继续404，等待structured-event session桥。 | implementation `502302066c506f8c7373daf14476009197cf1922`。Desktop tests=`85/0/0`，新增parser三路覆零与完整unary矩阵：ordinary bound window完成list/create/detail/route/mint/status/begin201/replay200/conversation/cancel202，secret response0；events两路404。Desktop all-target/all-feature Clippy `-D warnings`、macOS host、`x86_64-pc-windows-msvc` all-feature check与Tauri dependency guard全绿；parity仍`813/881/1694`、overlay=`1445/241/2/6`、strict=`159/0/0`，Cargo.lock/Grok/单package/workflow零变化。无UI/CSS/locale改动，未重跑Trunk/Browser/Engine，不冒充native-window/realtime证据；未运行CI/Actions。详见Batch71文档 |

| R146 | §3.1条4–5 / §5.1–§5.3 / §13.1–§13.4 / §15.1–§15.3 / §21.1条2 / §24 G4、G6（2026-08-30 Batch72：Desktop structured-event host bridge） | R145刻意让events/ws保持404，但`DesktopSession`文档又规定receiver只能在terminal后结束；此前application stream自然结束时pump直接退出并drop sender，形成静默EOF。一个真实window还要并行消费thread、channel activity、approval等closed stream，而broker label唯一；只用真实label会被duplicate拒绝，把label/actor/tenant交给renderer自报又会破坏§13.3。 | custom-protocol `Response<Vec<u8>>`不能伪装实时流；Tauri event也不能承载Screen。window在`subscribe().await`期间关闭并以同label重建时，旧请求若只在await前校authority会附着到新窗口。上游若错发stream family或另一thread event，不能因ApplicationService受信就直接投影给renderer。自然结束、host close、queue overflow与shutdown都必须显式terminal，sequence/gap不能静默丢。 | 新增macOS/Windows `tauri-host`限定的structured bridge：host以checked subscription id铸injective internal route，wire只含subscription/stream/sequence/gap/typed AppEvent/closed terminal，不回填authority；逐帧校sequence、stream family与exact thread后才进真实`tauri::ipc::Channel`。broker增加exact idempotent close及`upstream_ended`/`subscription_closed`，transport每session独立cancel且自然EOF先发terminal。`DesktopTauriProtocol`只用已绑定AuthContext开流，checked window binding nonce在await后复核；unbind同步关掉该实际window全部已登记route，renderer sink消失也drop取消。Screen仍构造性不进该桥。actual command注册、WASM host选择、真实native-window assembly、一个Webview的聚合队列/并发预算、Windows runtime/golden仍todo。 | implementation `c055bcbf749d8d0a37cc6c42cb03d8b2a61bab88`。Desktop default/all-feature=`80/0/0`与`98/0/0`，覆盖同窗多Channel、跨窗精确关闭、wrong family/thread拒绝、upstream/host/sink terminal、counter exhaustion、stale binding重建竞态；doc-test 1 ignored显式可见。all-target/all-feature Clippy `-D warnings`、macOS host与`x86_64-pc-windows-msvc` compile-only、Tauri dependency guard、fmt/diff全绿。parity仍`813/881/1694`、overlay=`1445/241/2/6`、strict recount=`159/0/0`；Grok tree/inventory、Cargo.lock、workflow、单package/零npm均不变。无UI/T-ID/依赖变化，未重跑Trunk/Browser/Engine，未运行CI/Actions；详见Batch72文档 |

| R147 | §5.1–§5.3 / §13.1–§13.4 / §15.3 / §21.1条2 / §24 G4、G6（2026-08-30 Batch73：Desktop structured-event actual commands） | R146只有Rust host bridge method，没有注册renderer可invoke的actual command。若open command一直await到stream terminal才返回，组件卸载时拿不到可关闭句柄；而JavaScript callback被unregister并不会让Tauri `Webview::eval(runCallback)`报错，host pump可继续存活到window关闭。R146 wire只在`openbot-desktop`，WASM若另写一份serde shape会形成第二契约。 | command若接受renderer自报window label可跨window关闭；只按全局subscription id关闭会形成枚举/越权。把内部`WindowAlreadyOpen(label)`直接作为invoke错误会泄露injective internal label。open先返回id后才spawn若顺序不闭合会出现receipt存在但pump未所有；反向spawn后无exact close仍泄漏。所有custom command必须显式枚举并与`generate_handler!`机械join，不能靠注释。 | 将command names、closed frame、stream/gap/terminal、open receipt与close request单源上收到wasm-safe `openbot-contracts::desktop`，Desktop只做本地gap/delivery映射。新增exact bridge/protocol close：只在host观察到的actual Webview label内移除对应id，unknown/already-finished/another-window统一false。注册Wry actual `openbot_structured_events_open/close`：open从`Webview`取label，经bound authority开流，先持有subscription、spawn后台Channel pump，再立即回host-minted id；close从同一Webview取label并消费closed receipt。错误只回stable code，不回内部label。公开二项command audit allowlist，单测与函数数、`generate_handler!`逐项join；`register_tauri_protocol`同时manage同一Arc、注册handler与custom scheme。WASM host selection、callback ordering/cleanup、真实window bind/unbind assembly、单Webview聚合256预算、Windows runtime/golden仍todo。 | implementation `61689c4a0169e25eda700bb3d4db668656fa44b8`。Contracts/Desktop default/all-feature=`98/80/102`且0失败；新增wire deny-unknown/authority0、exact cross-window close、stable error去label、Wry builder registration及command audit join。Contracts wasm32 check、all-target/all-feature Clippy `-D warnings`、macOS host、`x86_64-pc-windows-msvc` compile-only、Tauri dependency guard与fmt/diff全绿。parity=`813/881/1694`、overlay=`1445/241/2/6`、diff required revalidate=19且逐条展开；19条全因`openbot-contracts/src/lib.rs`粗前缀命中既有Agent/thread done，相关生产模块diff0，full Contracts已重跑，不冒充PG重跑。strict=`159/0/0`；Grok/Cargo.lock/workflow/单package/零npm不变。无UI/T-ID/依赖变化，未重跑Trunk/Browser/Engine，未运行CI/Actions；详见Batch73文档 |

| R148 | §3.1条4–5 / §5.1–§5.3 / §13.1–§13.4 / §15.3 / §21.1条2 / §24 G4、G6（2026-08-30 Batch74：WASM structured transport host selection） | R147已注册actual commands，但同一release WASM的thread仍无条件`EventSource`，channel roster与approval仍无条件WebSocket；Desktop custom scheme没有可构造的HTTP/WS endpoint，所以会持续重连不存在的Web socket。若按URL scheme猜host，Windows WebView2的custom scheme映射又可能表现为HTTP形态；Tauri 2.11.5唯一构造性标志是注入的`window.isTauri`。 | 零npm禁止引`@tauri-apps/api`；开启宽泛`window.__TAURI__`又扩大主Webview API面。Tauri Channel大payload经fetch可让callback按`index`乱序到达，且最后另发`{end,index}`；只按回调到达顺序消费会伪造sequence gap。open receipt与首帧无先后保证，unmount还可能发生在receipt前。仅unregister callback不会让host `Webview::eval`失败，必须exact close。直接把frame eval成JS对象会先把任意`u64>2^53-1`舍入，之后WASM再stringify也无法恢复。 | 新增wasm-only adapter，只读取钉版Tauri注入的`isTauri`与`__TAURI_INTERNALS__.invoke/transformCallback/unregisterCallback`五个exact字面，不启global Tauri、不增JS/npm。host Channel改为每次发送一条closed frame的JSON string，Rust/WASM直接serde，保留所有u64；host subscription counter以`2^53-1`为排他上界，到界fail-closed。adapter先按index bounded257重排并要求terminal后exact end，再校host receipt id、stream family、wire sequence/gap；receipt前frame bounded257，单frame JSON ≤1MiB。invoke resolve/reject closure交JS GC；connection Drop无论receipt已到或尚在flight都最终调用exact close，本地integrity fail同样close；`finished`只在terminal+end或显式failure完成。AppSidebar、Approval与Conversation三owner只在`isTauri=true`走typed command，否则原WebSocket/EventSource原样保留；各自沿既有refetch/backoff，Conversation由Effect StableValue所有connection，generation切换先Drop。真实Tauri window/session bind-unbind assembly、单Webview聚合256预算、Windows runtime/golden仍todo。 | implementation `b6520820f71a3cc59b0d134da0d7f95be5e218ae`。Contracts/Desktop/UI=`99/102/175`且0失败；UI新增5条adapter/三owner机械测试。Contracts/UI wasm32、三crateall-target/all-feature Clippy `-D warnings`、macOS host、Windows compile-only、Tauri guard与fmt/diff全绿。钉版tools verify后release/offline/locked bundle A/B八文件逐字相同；i18n=`782`、design=`104 Rust/74 icons`、CSS=`361`，bundle=`1849013/114965/740216/1/0`，WASM内open/close/internals/channel/isTauri各1。release in-app Browser证明普通Web三面仍`isTauri=undefined`：roster generation2、approval card1、transcript7，三页main/nav/h1唯一、visible alert/overflow/duplicate/log均0；这不是native证据。首次Trunk因ambient`NO_COLOR=1`在构建前拒绝，改closed true后绿；首次parity因root module粗前缀134违反，改由`api`拥有adapter后最终0违反、diff revalidate=9并逐条复核。parity=`813/881/1694`、overlay=`1445/241/2/6`、strict=`159/0/0`；Grok/Cargo.lock/workflow/单package/零npm不变。无T-ID/依赖/视觉变化，未运行CI/Actions；详见Batch74文档 |

| R149 | §13.1–§13.3 / §15.3 / §24 G6（2026-08-30 Batch75：单Webview structured aggregate budget） | R146为同一actual window的每条subscription铸injective internal broker label，再逐条调用`open_session`；但`EventBroker::open_window`会为每个internal label各建256格队列。于是roster/approval/thread三流不是“每窗口256”，而是最多768；renderer还可直接重复invoke open，把task、queue与terminal reserve继续相乘。window unbind只关已登记route，卡在`ApplicationService::subscribe().await`的pending open也没有同步取消。 | 把每流容量简单除以当前流数会在开/关流时改变既有队列语义，且不能处理并发open；只设process-global 256又会让一个窗口饿死另一个。permit若在send成功后立即释放，多个internal mpsc仍可合计超过256；若只在frame最终drop释放，则已出队但被消费端暂存的frame会错误占住queue预算。pending若不在await前预留，第257条并发open仍可全部穿进ApplicationService；同label关闭后重建还可能让旧open附着新窗口。 | 新增actual-window `WindowAggregate`：全部internal route共享一个256-slot `EventQueueBudget`，普通event在进入mpsc前原子取permit；`DesktopSession::next_frame`出队即释放，raw receiver绕过helper时最迟随frame drop释放，send failure/receiver drop同样回收；terminal不占256但每route原有专用reserve保持。live+pending subscription总数在await前以同一256上限预留，第257条不调用ApplicationService；pending guard以window token防同label重建，window close先摘aggregate、取消全部pending subscribe，再精确关闭registered route。`WindowAuthority`另带binding cancellation，堵住“先读authority、后发生unbind、再开始bridge open”的竞态；另一actual window拥有独立预算。 | implementation `24d9a5b1e051386b3ae4ed581e5054824da3a18f`。Desktop首次all-feature=`101/4/0`，暴露permit释放点与既有stale-window错误投影，修复后最终`106/0/0`，doc-test 1 ignored显式可见。新增正向/负向证明：两流各200个critical最终合计只入队256且第257个显式`queue_overflow` terminal；消费后256 permit全回；同窗前256条live与256条并发pending可进，第257条均在ApplicationService前拒绝；close可在不释放测试gate时取消256个pending且route/registry=0；另一window不被饿死。Desktop all-target/all-feature Clippy `-D warnings`、macOS host、`x86_64-pc-windows-msvc` compile-only、Tauri guard、fmt/diff全绿。parity=`813/881/1694`、overlay=`1445/241/2/6`、diff revalidate=0、strict=`159/0/0`；Grok tree/inventory、Cargo.lock/workflow、单package/零npm不变。无T-ID/依赖/UI/视觉变化，未重跑Trunk/Browser/Engine/golden，未运行CI/Actions；详见Batch75文档 |

| R150 | §6.1 / §13.1–§13.3 / §15.3 / §23.4 / §24 G6（2026-08-30 Batch76：品牌无关native window lifecycle primitive） | R147–R149已有custom scheme、commands、WASM选择和预算，但所有测试仍手工调用`bind_window`；production registration没有一个owner负责“verified authority先绑定→创建actual Webview→destroyed后exact unbind”。若等可发布`tauri.conf`与品牌一起补，权限时序、导航策略和失败回滚会继续没有可执行契约。 | build后才bind会让首个local asset/API抢跑401；build前bind但失败不回滚会留下无窗口authority。`CloseRequested`可被prevent，收到它就unbind会让仍存活窗口失权；只等renderer通知close又可伪造/遗漏。主Webview若允许remote navigation、`window.open`、download或devtools会扩大XSS后能力面。钉版Tauri明确规定macOS custom origin=`<scheme>://localhost`，Windows默认经Wry映射为`http://<scheme>.localhost`；只按一个形态过滤会在另一平台误拒或误放。另一方面，§23.4未给reviewed外部名称/bundle id/deep-link，不能用生命周期代码偷渡发行identity。 | 新增`VerifiedDesktopWindowAuthority`与`DesktopWindowLifecycle`：只接Rust已验证`AuthContext`，先bind host label，再用actual `WebviewWindowBuilder`创建custom-protocol窗口；build/registry失败先destroy/unbind回滚。production固定`devtools(false)`、exact local navigation、deny new-window/download；macOS只收native origin，Windows另收且只收Tauri 2.11.5文档规定的HTTP workaround，无userinfo/非默认port/错host。lifecycle作为managed state接进既有registration，只有真实`WindowEvent::Destroyed`才exact unbind，其他event不提前撤权；shutdown可一次撤全部authority。无`tauri.conf`、binary、capability、session source或产品identity。 | implementation `c424228a85b088f5167cdaa64493c6f6cf6e1b53`。Tauri dependency-free `test` feature仅作target-scoped dev依赖；MockRuntime真实build两个`WebviewWindow`，证明start URL/managed state、bind-before-content、invalid label build rollback、Focused不unbind、Destroyed exact且幂等、shutdown 2→0；纯origin矩阵同时覆盖macOS形态与Windows workaround负面。Desktop final=`109/0/0`，doc-test 1 ignored；Clippy首跑只因显式lifetime失败，消除后all-target/all-feature `-D warnings`绿；Windows中间编译暴露平台enum dead-code warning，改编译期bool后最终无warning。`x86_64-pc-windows-msvc` compile-only、Tauri guard、fmt/diff全绿；parity=`813/881/1694`、overlay=`1445/241/2/6`、revalidate=0、strict=`159/0/0`，Grok/Cargo.lock/workflow/单package/零npm不变。无T-ID/UI/视觉变化，未跑真实Wry event loop/Trunk/Browser/Engine/golden/CI/Actions；详见Batch76文档 |
...
| R156 | §5.1–§5.2 / §6.4 / §13.1 / §14.1 / §15.3 / §16.2–§16.4 / §24 G2、G6（2026-08-30 Batch82：PostgreSQL sidecar process lifecycle） | R154/R155已有可信bundle/lock/secret，却没有一个owner执行version→initdb→spawn→ready→shutdown；若Tauri setup自行拼Command，会重新出现PATH fallback、secret进argv/env/file和window先于DB。只spawn不做SCRAM query也不能证明HBA/password/port可用。 | `--version`若只看exit或不设deadline可接受错patch/遗留process；initdb password file/env/argv都会持久泄漏，stdin写本身也须deadline。无PG_VERSION的非空目录不能重跑initdb猜恢复。预占ephemeral port存在TOCTOU，只能让冲突启动失败，不能因此放宽到remote/hostname。ready必须持有exact child并用key-store secret真TCP SCRAM query；进程early-exit与timeout分型。Drop若先释放lock再kill会短暂双启动；反向无条件保留又会让clean shutdown永久stale。平台fixture的Homebrew绝对dylib/share不等于发行bundle。 | 新增显式`postgres-supervisor` feature（只加既有tokio-postgres direct edge）与`PostgresSidecarSupervisor`。start先验证app-data/data-dir exact direct child、Unix私权，再create lock；三个manifest-verified program依次在5s内跑`--version`，要求`<program> (PostgreSQL) 17.11`且最多一个bounded printable vendor suffix、stdout/stderr恰一条，version失败在key-store写前。持锁取R155 secret；data-dir空才initdb，缺PG_VERSION但非空或version非17拒绝。initdb以env_clear/LC_ALL=C/verified binary、stdin pipe两次写secret、auth-host SCRAM/local reject/no-locale，write+wait整体30s，secret不进argv/env/file。每启只写private `postgresql.auto.conf`/`pg_hba.conf`：numeric127、SSL off、all host SCRAM、Unix socket disabled；direct spawn verified postgres并持Child，10s内循环检查child identity与tokio-postgres NoTls/TCP SCRAM `SELECT 1`才ready。`RunningPostgresSidecar`持child+lock+secret+bundle，暴露borrowed redacted connection；shutdown只用verified pg_ctl fast/wait再等exact child成功后释放lock。start失败确认child死亡才释锁；unclean Drop先preserve stale lock再kill，显式repair前下一start报held。尚不依赖infra/不执行Batch79 schema/package，保持Windows可编译。 | implementation `cd66ed46c0255137bac2de0c91f5077503b48461`。Desktop完整=`119/0/1`，sidecar子集=`10/0/1`；ignored真实fixture宿主单跑=`1/0/0`：将Homebrew17.11三binary复制进test manifest（动态依赖/share仍外部，非release证据），fresh verified versions→stdin initdb→SCRAM ready，配置/raw-secret canary0，fast shutdown释放lock；existing restart同secret再ready/shutdown；第三次unclean Drop后port关闭、stale lock存在且第四次start held，确认进程死后显式repair，bundle/app/data目录0。fault矩阵另证wrong version写secret0/lock0、initdb failure持久secret1但process0/lock0、early exit显式且lock0、partial/PG16/symlink/private/config规则。sandbox真实fixture首跑InitdbFailed不计；宿主首次再暴露IPv6 HBA reject违反all-host-SCRAM，改SCRAM后最终绿。macOS/Windows all-target/all-feature Clippy绿（首跑分别collapsible-if与target unused-mut后修）；Linux feature-only compile绿且既有2 warning。Cargo=`825→825`，只增既有tokio-postgres direct edge；key-store/Tauri/deny guards绿。parity=`813/881/1694`、required revalidate0、overlay=`1444/242/2/6`，strict=`159/0/0`；Grok/workflow/单package/npm不变。未实现/未跑release PG binary build/sign/relocatable依赖、Windows process/key-store真机、Batch79 schema/principal/package组合、Tauri setup/window、backup/upgrade；port reservation冲突fail-closed不自动retry；本条不关闭G2/G6整关且无新T-ID/UI/CI/Actions；详见Batch82文档 |

| R157 | §5.1–§5.2 / §6.1、§6.4–§6.5 / §13.1–§13.3 / §14.1 / §15.3 / §16.2–§16.4 / §24 G2、G6（2026-08-30 Batch83：PostgreSQL bootstrap composition） | R156只返回SCRAM-ready `postgres`库child，尚未创建业务库，也未消费R153 shared bootstrap；Tauri若自己从borrowed secret拼`DatabaseConfig`，会留下password String、可能用错admin role，并可在live attestation前写库。Desktop直接依赖完整infra又会把Server TLS/SAML/xmlsec拖入Windows图。 | `CREATE DATABASE`不是可回滚事务，错误必须重读权威row分型unknown commit；existing库的owner/encoding/locale/access若不闭集，后续schema权威可被别的role改写。Tenant scope必须在建库前拒绝。attestation若只是调用顺序注释，未来仍可绕过；需要unattested/attested typestate。pool与child若分属不同owner，失败或退出会提前释放start lock。Batch82 `initdb`实际admin=`desktop_admin`，重复字面量已真实造成28P01。另一方面，feature-gating若只关SAML仍会经rustls/ring阻塞Windows交叉图。 | contracts单源固定非秘密`DESKTOP_LOCAL_POSTGRES_ADMIN_USER=desktop_admin`，initdb/ready/connection/infra/DB owner全引用。infra新增`desktop-local`最小图，Server显式`server-runtime/server-sso`；Desktop三平台图`ring/rustls/samael/openssl-sys=0`，Server仍含。composition先比exact installation path与package tenant；sidecar secret只复制进startup `SecretBytes`。`connect_for_attestation`只产`UnattestedDesktopLocalAdmin`；只有R153 live attestation可转`AttestedDesktopLocalAdmin`，随后固定创建/核验`openbot`（owner、UTF8、C locale、non-template、allowconn、limit=-1），create error重读exact row并显式`ReconciledAfterCreateError`。再复用shared initialize→principal→package。`RunningDesktopLocalDataPlane`共同持pool/installation/report/child；失败关闭pool并verified shutdown，普通Drop保留stale lock。尚不创建ApplicationService/window。 | implementation `798cdbe59c0f952c182cacae14002b03bfc3e10b`。Desktop=`120/0/2`、infra lib=`321/0/0`、Server=`216/0/0`；真实Homebrew17.11 composition=`1/0/0`：wrong tenant后DB不存在/lock0，随后Created+Fresh+admin+membership1，restart Existing+RustManaged+grant0，两次clean，目录0。为155条粗owner revalidate，最终以SCRAM临时集簇跑infra全部tests+ignored=`514/0/0`、agent runtime=`8/0/0`；过程中ENOSPC与trust-HBA自检失败均不计，修复既有component 13项过滤计数8/9→10/11及0021/0022历史fixture误用apply-latest。macOS Desktop/infra/Server与Windows Desktop+unsafe边界Clippy绿；Linux feature compile绿有既有2 warning。Cargo825不变，guards绿；parity=`813/881/1694`、overlay=`1289/397/2/6`、0违反、strict=`159/0/0`。未实现/未跑release PG build/sign、Windows process/key-store真机、真实Tauri app-data/background setup、production ApplicationService/window、backup/upgrade；本条不关闭G2/G6整关，无新API/UI/T-ID/CSS/locale/CI/Actions；详见Batch83文档 |

| R158 | §5.1–§5.2 / §6.1、§6.4 / §8.6 / §9.2 / §13.1–§13.3 / §16.2–§16.4 / §24 G2、G6（2026-08-30 Batch84：Desktop Vault master key） | R157 data-plane已能承载application adapters，但credential Vault/audit/run assertion/MCP OAuth都需要master-derived material；当前仅PG SCRAM secret在OS key store，Server仍从`KEY_ENCRYPTION_KEY`组装。若Desktop复用env/app-data file或测试常量，就直接违反§6.4；若各模块各写Keychain/Credential Manager，又会产生第二平台安全真源。 | master首次双进程生成必须由instance start lock fencing，否则不同key会让同一数据库密文永久分裂；existing corrupt不能覆盖。stored value需显式format version以支持未来rotation。master明文若Clone/Serialize/String或长期保留，会扩大crash/log面。PG与Vault不能共享同一item/格式。audit/MCP derivation label若Server/Desktop复制，会造成跨发行验签漂移；但Server历史从raw配置字节（常见44-byte base64文本）HMAC，不能错误收紧为AES 16/24/32形状。macOS/Windows API应只有一个consumer；Linux tier-2没有Secret Service时不得伪造runtime。 | 新增共享`OsSecretStore`，production macOS Keychain consumer恰一、Windows仍经唯一unsafe Cred wrapper；PG以blanket adapter保留自身64hex格式。Vault service/account独立，account=`desktop-vault-master-v1-<instance>`；item exact 33 bytes=`version1+32-byte CSPRNG`，corrupt写0，new write→readback→CT。唯一production入口是仍持child/lock的`RunningDesktopLocalDataPlane::load_application_key_material`。master进入`SecretBytes`，导出tenant-bound `CredentialRecordVault`、audit key、`RemoteRunAssertionSigner`与MCP OAuth state后drop。audit/MCP labels上收到domain closed enum/function并钉旧Server向量；Server改用同一函数、raw master兼容不变，hmac移出Server production direct graph。macOS private Keychain真seal/restart/open；Windows同格式用例ignored compile，Linux只compile无adapter。 | implementation `fc2085cffd8ceeb096ba576f228e41a88e5172aa`。Domain=`371/0/0`、Desktop=`123/0/2`、Server lib/bin=`216/0/0+7/0/0`；两条macOS private Keychain（PG+Vault）真跑，Vault首次/restart/corrupt/readback mismatch与derived-key分域全绿；Homebrew PG+data-plane+Vault跨重启=`1/0/0`。macOS完整Clippy与Windows Desktop+unsafe target Clippy绿；Windows Vault/Credential runtime未跑。Linux `desktop-vault` compile绿且既有2 warning。guard证单Keychain consumer/单Cred FFI/labels单源/PG-Vault格式隔离；Cargo825不变（Desktop dev加既有uuid，Server production去既有hmac direct）；parity=`813/881/1694`、overlay=`1289/397/2/6`、0违反、strict=`159/0/0`。未实现Linux Secret Service、key rotation/recovery、production ApplicationService、Tauri path/background/window、release/backup；本条不关闭Desktop全Vault或G2/G6整关，无新API/UI/T-ID/CSS/locale/CI/Actions；详见Batch84文档 |

| R159 | §5.1–§5.2 / §6.1、§6.4 / §8.6 / §9.2 / §13.1–§13.3 / §16.2–§16.4 / §24 G2、G6（2026-08-30 Batch85：ApplicationService adapter assembly） | R158已给Desktop提供真实pool/auth与全部application key material，但production PostgreSQL adapter wiring仍有257行只存在于Server `main.rs`。若Tauri复制People/Policy/Thread/Run/Memory/Agent/Tool/MCP/Drive/Component/Channel wiring，就会出现两个可漂移的`OpenBotApplication` owner；若反向把env解析、Axum或window放入Infra，又违反transport只依赖唯一ApplicationService。Desktop若从sidecar secret重建Server `DatabaseConfig`，还会制造password `String`与环境配置第二真源。 | 共享的是业务adapter composition，不是host：输入必须在host侧先验证并typed化，输出必须保留所有background lifecycle owner；环境、HTTP listener、native window和built-in Agent host继续由各自host拥有。`ApplicationService` production constructor只能有一个owner。dedicated LISTEN需要同一数据库凭据，但Desktop只能从持锁child借用secret bytes，并以Debug脱敏的opaque hand-off跨feature；不能为了一个配置值拉入Server SSO/OpenSSL图。 | 新增`openbot-infra::application_assembly`，显式输入deployment/tenant/policy/Vault/HMAC/provider/remote probe，统一构造全部PostgreSQL application adapters、唯一`Arc<dyn ApplicationService>`及run/MCP/component/callback lifecycle handles；生产源码零env/Axum/Tauri/Webview/listener。Server删重复wiring并精确调用一次。新增`thread_listener`共享opaque wrapper：Desktop固定numeric loopback/`openbot`/`desktop_admin`/64-byte lowercase hex且无password `String`，Server从既有config转换；`desktop-local`不启`server-runtime`。guard锁owner=1、Server consumer=1、Desktop listener=1与forbidden=0。 | implementation `5ba28673d6d03340095d74b2511d9438f5dcfb5f`。Infra=`323/0/0`、Server lib/bin=`216/0/0+7/0/0`、Desktop=`123/0/2`；真实PG17.11 shared assembly执行`GetCurrentUser`=`1/0/0`，真实Desktop sidecar→bootstrap→listener-config→clean shutdown=`1/0/0`。macOS全目标Clippy与Windows Desktop+唯一unsafe crate target Clippy绿；Linux `desktop-vault` compile绿并仅有既有2 warning。沙箱`initdb` SysV EPERM与Desktop首编译feature边界失败均不计成功，后者拆共享module后重跑绿。Cargo825不变；parity=`813/881/1694`、overlay=`1289/397/2/6`、0违反；本仓recount=`71/0`，上游路径未设故88 skipped且strict未跑。Grok tree/inventory、单package/npm、workflow不变。Desktop尚未消费完整assembly，真实Tauri app-data/background/window-last、release PG build-sign、Windows runtime、Linux Secret Service仍todo；不关闭G2/G6整关，无新API/UI/T-ID/CSS/locale/CI/Actions；详见Batch85文档 |

| R160 | §5.1–§5.2 / §6.1、§6.4–§6.5 / §7.1–§7.5 / §8.6 / §9.2 / §13.1–§13.4 / §14.1 / §16.2–§16.4 / §24 G2、G4、G6（2026-08-30 Batch86：Tauri background/Agent/window-last assembly） | R159已有可复用ApplicationService，但Tauri custom protocol必须在Builder阶段注册，权威app-data path却只在setup可得；照旧API会逼迫调用方在authority/DB前构造protocol。`tauri.conf`静态window又会先于setup创建。只接application不接RunRelay/BuiltInAgentRuntime会让UI产生无人消费的dispatch outbox；shared assembly硬编码PG preference又会推翻Batch16 Desktop local-file真源。最后窗关闭若让Tauri直接退出，MCP/Agent/PG start lock来不及有序释放。 | event loop必须以零窗启动；pending protocol只能fail-closed，且ready owner不可替换。所有阻塞OS-store/PG初始化在Tauri runtime worker完成，首窗是最后一步。host差异通过typed port输入而非复制OpenBotApplication。Desktop provider不得复用Server env/managed secret，只有HTTPS+exact CIDR+Vault；正常package含built-in/remote Agent，缺consumer时禁止开窗。退出必须先撤window authority/stream，再停transport、relay/Agent、MCP，最后关pool/verified child；ExitRequested在终态前必须阻止。 | 新增`desktop-local-runtime`与`DesktopLocalTauriBuilder`：外部Context若有`create=true` window即拒；setup唯一读`AppHandle::path().app_data_dir()`并异步执行authority→verified sidecar→fixed DB/bootstrap→application keys/policy→shared assembly→Desktop Agent host/RunRelay→single-install protocol→verified first window。`DesktopTauriProtocolSlot` pending返回503、二次install拒绝。shared assembly改收`UiPreferenceAdministration`，Server传PG、Desktop传app-data原子文件。Desktop OpenAI base只收HTTPS/no-userinfo-query-fragment，Responses/Chat双endpoint、64MiB/30s、stored credential only；remote AG-UI同SafeDialer，managed source显式unsupported。RunRelay与thread stream共用redacted listener wrapper。last Destroyed触发authority→transport→relay/Agent→MCP/adapters→data-plane/sidecar，完成后才exit。 | implementation `35ab92b12b7ae85ec8bce0c02366ea0a1c0c8b76`。Desktop=`130/0/3`、Infra=`323/0/0`、Server=`216/0/0+7/0/0`；真实Homebrew PG17.11 background纵向=`1/0/0`：OS-store两item写2、GetCurrentUser、local preference、mint→BeginRun后真实consumer写`failed/provider_authentication`、clean lock/dirs0。shared assembly real PG=`1/0/0`。MockRuntime/guard证actual path resolver、零静态窗、phase/last-Destroyed/single protocol、ordered startup/shutdown；不是Wry真机。macOS Clippy绿；Windows最小图绿但full cross在ring/MSVC CRT `assert.h`前失败，Linux最小compile既有2 warning。Cargo825（只增既有openbot-agent/url direct edge）；parity=`813/881/1694`、overlay=`1289/397/2/6`、0违反；recount=`71/0/88 skipped`且strict未跑。正式tauri.conf/capability/identity/release binary、release PG build-sign、Windows full-runtime/真机、Linux Secret Service、真实Wry/golden仍todo；不关闭G2/G4/G6整关，无API/UI/T-ID/CSS/locale/CI/Actions；详见Batch86文档 |

| R161 | §7.2、§7.4 / §8.4 / §14.3 / §15.3 / §24 G3、G4（2026-08-30 Batch87：run-wide normalized token accounting） | R70只把`OPENBOT_PROVIDER_MAX_OUTPUT_TOKENS`作为每次sampling上限；真实tool loop已能多次sampling，但`RunRuntime`没有跨sampling usage真源，进程内相加既不能跨commit-unknown精确回放，也无法在多副本/恢复时审计。直接硬编码vendor价格或把每sampling cap冒充用户费用上限同样违反第一真源：仓内没有model/version→price的权威、带时点provenance来源。 | Usage必须先按provider normalized三元组验证，再以run lease/fencing与连续sampling index原子记账；同一last index+同值精确回放，异值/跳号/cap漂移冲突。超预算sampling已真实消耗，必须先记账再返回超限，不能为了“库内不超过上限”低报。首个权威per-sampling cap固定后不可漂移；其乘以`TOOL_STEPS+1=9`只作为不引入新配置的derived consistency ceiling，不称用户cost budget。价格provenance、用户费用上限、并发tool与computer runtime继续明确todo。 | native0024只给`runs`追加9列与3个具名CHECK；input/output/total aggregate、next/last sampling和immutable ceiling均由PostgreSQL事务维护。Agent每轮context校cap不漂移，usage先写runtime；per-sampling超限也保留实际用量，run超限落稳定`failed/run_token_budget_exceeded`。implementation `7592caeae379ef78fefff06259c6ecfbd7614c34`。PG17.11 fixture regeneration开/关=`1+1/0/0`，schema=`45表/437列/320 NOT NULL/246约束/91索引/4触发器/4枚举/1函数/0扩展`、SHA=`349ff3f9573ee97cd588d717ea107d3b3ed1c5cf677ff75dc637b4e104de1031`；run runtime=`5/0/0`，native0016/0023/0024=`3+1+1/0/0`。Agent/Application/Domain/Infra=`35/153/371/323`，四crate all-target/all-feature Clippy、fmt/diff绿。parity=`813/881/1694`、fixtures=`18/22/40`、overlay=`1283/403/2/6`、0违反；recount=`71/0/88 skipped`，strict未跑。Cargo/Grok/npm/workflow不变；不关闭完整run-wide budget或G4整关；详见Batch87文档。 |
