...
- [ ] **G2**：整关未通过；以下子项已经有本机机械证据：
  ...
  - [ ] 独立 SAML/XSW 与整体安全外审、Server KMS/HSM、Windows 原生构建；Desktop Local installed-app OAuth、Drive/browser/file/shell 与真实 approval 等 G4 余面仍缺。
...
- [ ] **G4**：整关未通过；以下 Rust built-in Agent 子面已有本机机械证据：
  ...
  - [ ] provider gate 要求的三家 recorded vendor trace 仍为 **0/3**，本批未使用 live vendor credential；human proof-of-intent/approval GUI、完整 run-wide token/cost/并发/computer budget、Desktop Local installed-app client/system browser/random loopback callback、run/user cancellation 向 RMCP notification 传播、MCP 专用 private egress与 admin add/refresh/grant/effect 完整 UI、用户创建 remote Agent lifecycle/customer auth、interrupt/resume 与其余事件 durable/UI projection、Drive 及 browser/file/shell executor 尚未闭合。
...
当前总台账：parity **394/1661 done（1267 todo）**，fixtures **12/34 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
