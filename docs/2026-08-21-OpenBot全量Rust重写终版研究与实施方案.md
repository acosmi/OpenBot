...
- [ ] **G3**：整关未通过；以下 native data base 已有机械证据：
  ...
  - [ ] 50ms/8KiB accumulator 已有纯 Rust 边界测试但尚未接真实 provider producer；WebSocket parity、history、terminal/chunk writer、outbox relay、memory API/GUI、Intelligence importer/checksum 尚未闭合。
...
当前总台账：parity **280/1646 done（1366 todo）**，fixtures **10/32 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
