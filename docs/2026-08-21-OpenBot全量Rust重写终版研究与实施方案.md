...
- [ ] **G3**：整关未通过；以下 native data base 已有机械证据：
  ...
  - [ ] ApplicationService thread/message/run 同事务 append、50ms/8KiB chunk、LISTEN/NOTIFY live、SSE/WS reconnect、memory API/GUI、Intelligence importer/checksum 尚未闭合。
...
当前总台账：parity **269/1645 done（1376 todo）**，fixtures **10/32 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
