...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] tokens.toml 单源生成 CSS/Rust、Inter 4.1 随包、74 项 icon manifest/SVG 双向闭合；i18n en/zh-CN 360 叶键及占位符 exact；WASM gzip/CSS/fonts 与零内联脚本预算绿；
  ...
  - [x] UI ledger 的27条primitive子账全done；46条Tabler→Lucide经第一真源→icons.toml→ledger三向join关闭；
    layout 组 detail-panel/page-shell/row-mark/stagger 四条业务又有生产实现与本机证据。当前UI=`77/75/152`；
    Google Drive brand、其余41业务/runtime/golden保持todo；
  ...
  - [ ] reviewed 外部产品名/bundle id/deep-link 后的 `tauri.conf.json`/binary、真实 window lifecycle/multi-window integration、macOS arm64/Windows x64 原生发行构建，以及其余41业务组件/31route、1 brand icon、6runtime替代、110 Web + 两平台各54 golden、完整axe/键盘E2E尚未闭合；
...
当前总台账：parity **513/1672 done（1159 todo）**，fixtures **15/37 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
