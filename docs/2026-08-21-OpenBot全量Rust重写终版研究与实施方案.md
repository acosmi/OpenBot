...
这条最初闭合的是**通用 application/infra/agent gateway 边界**。R71 后首个真实集合成员 `remember` 已以 production PostgreSQL effect 穿完：fresh ACL → metadata/schema → CEL → decision+attempt → capability CAS → `origin=remember_tool` memory → outcome+audit → durable tool pair → 下一次 sampling。Fact provenance、owner、Bot/thread target 都从当前 run/DB 取得；模型参数没有自报 ID 的位置。`AuthGeneration` 随不可序列化 execution scope 进入 executor，并在 memory INSERT 同一事务再次比较 generation/revoked/role，堵住 capability mint 后撤权竞态。browser/file/shell、MCP、Drive 仍须逐个实现并只能接收 `AuthorizedToolCall`；human proof-of-intent/approval GUI 尚未闭合，所以本条仍不能写成“G4 整关通过”。（§28.1 R71）
...
- [ ] **G2**：整关未通过；以下子项已经有本机机械证据：
  ...
  - [ ] 独立 SAML/XSW 与整体安全外审、Server KMS/HSM、Windows 原生构建；真实 G4 executor 也仍缺。
- [ ] **G3**：整关未通过；以下 native data base 已有机械证据：
  ...
  - [ ] Memory GUI，以及实际 legacy exporter/production bundle 三次演练尚未闭合。
- [ ] **G4**：整关未通过；以下 Rust built-in Agent 子面已有本机机械证据：
  ...
  - [x] package-backed `remote_ag_ui` lifecycle/text 生产竖切：权威 route→唯一 SafeDialer POST/SSE→decoder→durable semantic chunk/assistant/terminal；RunAgentInput 已带 DB-clock 10 分钟 signed run assertion 与 canonical empty tool-set hash；
  - [x] per-Agent callback token issue/rotate/revoke：`obot_agt_`+32-byte CSPRNG、DB hash-only、fresh Origin、owner/admin/fresh generation、mutation+audit 同事务；callback 同验 token/assertion/Bot/actor/run/lease/tool-set，当前 empty grant 的任何 tool 均在 executor 前 404 + refusal audit；共享 `AGENT_TOOL_TOKEN` 构造性不存在；
  - [ ] provider gate 要求的三家 recorded vendor trace 仍为 **0/3**，本批未使用 live vendor credential；human proof-of-intent/approval GUI、完整 run-wide token/cost/并发/computer budget、非空 remote tool grant 与 callback success/outcome、跨副本 call sequence、customer outbound auth header、用户创建 remote Agent lifecycle、interrupt/resume 与其余事件 durable/UI projection、RMCP/Drive 及 browser/file/shell executor 尚未闭合。
...
当前总台账：parity **352/1660 done（1308 todo）**，fixtures **10/32 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
