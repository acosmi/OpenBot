...
>
> 文档状态：终版实施基线 v4（v3 + 2026-08-28 第三轮就地修订 R115–R125 + 2026-08-29–30 实施裁决 R126–R151，修订清单见 §28.1，修订方法与真源优先级见 §28.5；`docs/2026-08-28-OpenBot-TauriGUI-ElectronChromium-GrokBot大面积Rust迁移-v4修订计划-用户裁决版.md` 已被吸收，只作历史记录）
>
...
- [ ] **G4**：整关未通过；以下 Rust built-in Agent 子面已有本机机械证据：
  ...
  - [x] 用户创建 Agent lifecycle/customer Authorization backend：七条fresh same-origin HTTP写面只收closed
    ...
    source→PostgreSQL principal/channel provisioning→lifecycle setup、可发布binary、真实Wry native runtime、
...
- [ ] **G6**：整关未通过；以下 Web GUI 地基已有本机机械证据：
  ...
  - [x] data-backed `/agents` surface：固定上游`mine`与`!mine && public`两组、144×180 AgentCard、
    ...
    的closed持久化与AuthContext。T-UI-0126 formal golden、source的PG provisioning/setup接线、Desktop Remote
    session、真实Wry native-window journey与Windows runtime仍todo；
