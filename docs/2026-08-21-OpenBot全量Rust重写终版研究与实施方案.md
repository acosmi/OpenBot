...
- [ ] **G4**：整关未通过；以下 Rust built-in Agent 子面已有本机机械证据：
  ...
  - [x] package-backed `remote_ag_ui` lifecycle/text 生产竖切：权威 route→唯一 SafeDialer POST/SSE→decoder→durable semantic chunk/assistant/terminal；无 signed run assertion 时 `tools=[]` 且非空工具在网络前拒绝；
  - [ ] provider gate 要求的三家 recorded vendor trace 仍为 **0/3**，本批未使用 live vendor credential；human proof-of-intent/approval GUI、完整 run-wide token/cost/并发/computer budget、remote callback token/run assertion/tool grant、用户创建 remote Agent lifecycle、interrupt/resume 与其余事件 durable/UI projection、RMCP/Drive 及 browser/file/shell executor 尚未闭合。
...
当前总台账：parity **325/1660 done（1335 todo）**，fixtures **10/32 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
