...
- [ ] **G3**：整关未通过；以下 native data base 已有机械证据：
  ...
  - [ ] 50ms/8KiB accumulator 已接 `DurableTextRun`，但尚无真实 provider producer；真实 Agent consumer、`remember` tool、Memory GUI，以及实际 legacy exporter/production bundle 三次演练尚未闭合。
...
当前总台账：parity **293/1653 done（1360 todo）**，fixtures **10/32 done（22 todo）**。勾选只表示整项判据已经通过；局部代码存在但整关未闭合时不得勾整关。
